...
static struct nbrec_meter *
nbrec_meter_cast(const struct ovsdb_idl_row *row)
{
    return row ? CONTAINER_OF(row, struct nbrec_meter, header_) : NULL;
}

static struct nbrec_meter_band *
nbrec_meter_band_cast(const struct ovsdb_idl_row *row)
{
    return row ? CONTAINER_OF(row, struct nbrec_meter_band, header_) : NULL;
}
...
static struct nbrec_port_group *
nbrec_port_group_cast(const struct ovsdb_idl_row *row)
{
    return row ? CONTAINER_OF(row, struct nbrec_port_group, header_) : NULL;
}
...
nbrec_acl_table_get(const struct ovsdb_idl *idl)
{
    return (const struct nbrec_acl_table *) idl;
}

const struct nbrec_acl *
nbrec_acl_table_first(const struct nbrec_acl_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_acl_first(idl);
}


const struct nbrec_acl *
nbrec_acl_table_track_get_first(const struct nbrec_acl_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return nbrec_acl_track_get_first(idl);
}
...
static void
nbrec_acl_parse_meter(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct nbrec_acl *row = nbrec_acl_cast(row_);

    if (datum->n >= 1) {
        row->meter = datum->keys[0].string;
    } else {
        row->meter = NULL;
    }
}
...
static void
nbrec_acl_unparse_meter(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}
...
/* Searches table "ACL" for a row with UUID 'uuid'.  Returns
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct nbrec_acl *
nbrec_acl_table_get_for_uuid(const struct nbrec_acl_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_acl_get_for_uuid(idl, uuid);
}
...
/* Causes the original contents of column "meter" in 'row' to be
 * verified as a prerequisite to completing the transaction.  That is, if
 * "meter" in 'row' changed (or if 'row' was deleted) between the
 * time that the IDL originally read its contents and the time that the
 * transaction aborts and ovsdb_idl_txn_commit() returns TXN_TRY_AGAIN.
 *
 * The intention is that, to ensure that no transaction commits based on dirty
 * reads, an application should call this function any time "meter" is
 * read as part of a read-modify-write operation.
 *
 * In some cases this function reduces to a no-op, because the current value
 * of "meter" is already known:
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     nbrec_acl_insert()).
 *
 *   - If "meter" has already been modified (with
 *     nbrec_acl_set_meter()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * nbrec_acl_set_meter() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
nbrec_acl_verify_meter(const struct nbrec_acl *row)
{
    ovsdb_idl_txn_verify(&row->header_, &nbrec_acl_col_meter);
}
...
/* Returns the "meter" column's value from the "ACL" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 *
 * 'key_type' must be OVSDB_TYPE_STRING.
 * (This helps to avoid silent bugs if someone changes meter's
 * type without updating the caller.)
 *
 * The caller must not modify or free the returned value.
 *
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "meter" member in nbrec_acl. */
const struct ovsdb_datum *
nbrec_acl_get_meter(const struct nbrec_acl *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &nbrec_acl_col_meter);
}
...
/* Sets the "meter" column from the "ACL" table in 'row' to
 * the 'meter' set.
 *
 * If "meter" is null, the column will be the empty set,
 * otherwise it will contain the specified value.
 *
 * The caller retains ownership of the arguments. */
void
nbrec_acl_set_meter(const struct nbrec_acl *row, const char *meter)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    if (meter) {
        datum.n = 1;
        datum.keys = &key;
        key.string = CONST_CAST(char *, meter);
    } else {
        datum.n = 0;
        datum.keys = NULL;
    }
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &nbrec_acl_col_meter, &datum);
}
...
/* Adds the value 'new_value' to the "meter" set column from the "ACL" table
 * in 'row'.
 *
 */
void
nbrec_acl_update_meter_addvalue(const struct nbrec_acl *row, const char *new_value)
{
    struct ovsdb_datum *datum;
...
/* Deletes the value 'delete_value' from the "meter" set column from the
 * "ACL" table in 'row'.
 *
 */
void
nbrec_acl_update_meter_delvalue(const struct nbrec_acl *row, const char *delete_value)
{
    struct ovsdb_datum *datum;
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_acl_add_clause_meter(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const char *meter)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
...
struct nbrec_acl *
nbrec_acl_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_acl);
    return ALIGNED_CAST(struct nbrec_acl *, ovsdb_idl_index_init_row(index));
}

struct nbrec_acl *
nbrec_acl_index_find(struct ovsdb_idl_index *index, const struct nbrec_acl *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_acl);
    return nbrec_acl_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
nbrec_acl_index_compare(
    struct ovsdb_idl_index *index, 
    const struct nbrec_acl *a, 
    const struct nbrec_acl *b)
{
    return ovsdb_idl_index_compare(index, &a->header_, &b->header_);
}

struct ovsdb_idl_cursor
nbrec_acl_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_acl);
...
struct ovsdb_idl_cursor
nbrec_acl_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct nbrec_acl *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_acl);
...
struct nbrec_acl *
nbrec_acl_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return nbrec_acl_cast(ovsdb_idl_cursor_data(cursor));
}
...
nbrec_acl_index_set_action(const struct nbrec_acl *row, const char *action)
{
    ...
    key->string = CONST_CAST(char *, action);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_acl_columns[ NBREC_ACL_COL_ACTION ], &datum, &nbrec_table_classes[NBREC_TABLE_ACL]);
}
...
nbrec_acl_index_set_direction(const struct nbrec_acl *row, const char *direction)
{
    ...
    key->string = CONST_CAST(char *, direction);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_acl_columns[ NBREC_ACL_COL_DIRECTION ], &datum, &nbrec_table_classes[NBREC_TABLE_ACL]);
}
...
nbrec_acl_index_set_log(const struct nbrec_acl *row, bool log)
{
    ...
    key->boolean = log;
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_acl_columns[ NBREC_ACL_COL_LOG ], &datum, &nbrec_table_classes[NBREC_TABLE_ACL]);
}
...
nbrec_acl_index_set_match(const struct nbrec_acl *row, const char *match)
{
    ...
    key->string = CONST_CAST(char *, match);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_acl_columns[ NBREC_ACL_COL_MATCH ], &datum, &nbrec_table_classes[NBREC_TABLE_ACL]);
}
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_acl_index_set_meter(const struct nbrec_acl *row, const char *meter)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof (union ovsdb_atom));
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_acl_columns[ NBREC_ACL_COL_METER ], &datum, &nbrec_table_classes[NBREC_TABLE_ACL]);
}
...
nbrec_acl_index_set_name(const struct nbrec_acl *row, const char *name)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_acl_columns[ NBREC_ACL_COL_NAME ], &datum, &nbrec_table_classes[NBREC_TABLE_ACL]);
}
...
nbrec_acl_index_set_priority(const struct nbrec_acl *row, int64_t priority)
{
    ...
    key->integer = priority;
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_acl_columns[ NBREC_ACL_COL_PRIORITY ], &datum, &nbrec_table_classes[NBREC_TABLE_ACL]);
}
...
nbrec_acl_index_set_severity(const struct nbrec_acl *row, const char *severity)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_acl_columns[ NBREC_ACL_COL_SEVERITY ], &datum, &nbrec_table_classes[NBREC_TABLE_ACL]);
}
...
struct ovsdb_idl_column nbrec_acl_columns[NBREC_ACL_N_COLUMNS] = {
    ...
    [NBREC_ACL_COL_METER] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = nbrec_acl_parse_meter,
         .unparse = nbrec_acl_unparse_meter,
    },
...
nbrec_address_set_table_get(const struct ovsdb_idl *idl)
{
    return (const struct nbrec_address_set_table *) idl;
}

const struct nbrec_address_set *
nbrec_address_set_table_first(const struct nbrec_address_set_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_address_set_first(idl);
}


const struct nbrec_address_set *
nbrec_address_set_table_track_get_first(const struct nbrec_address_set_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return nbrec_address_set_track_get_first(idl);
}
 ...
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct nbrec_address_set *
nbrec_address_set_table_get_for_uuid(const struct nbrec_address_set_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_address_set_get_for_uuid(idl, uuid);
}
...
struct nbrec_address_set *
nbrec_address_set_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_address_set);
    return ALIGNED_CAST(struct nbrec_address_set *, ovsdb_idl_index_init_row(index));
}

struct nbrec_address_set *
nbrec_address_set_index_find(struct ovsdb_idl_index *index, const struct nbrec_address_set *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_address_set);
    return nbrec_address_set_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
nbrec_address_set_index_compare(
    struct ovsdb_idl_index *index, 
    const struct nbrec_address_set *a, 
    const struct nbrec_address_set *b)
{
    return ovsdb_idl_index_compare(index, &a->header_, &b->header_);
}

struct ovsdb_idl_cursor
nbrec_address_set_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_address_set);
    return ovsdb_idl_cursor_first(index);
}
...
struct ovsdb_idl_cursor
nbrec_address_set_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct nbrec_address_set *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_address_set);
    return ovsdb_idl_cursor_first_eq(index, &target->header_);
}
...
struct ovsdb_idl_cursor
nbrec_address_set_cursor_first_ge(
    struct ovsdb_idl_index *index, const struct nbrec_address_set *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_address_set);
    return ovsdb_idl_cursor_first_ge(index, &target->header_);
...
struct nbrec_address_set *
nbrec_address_set_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return nbrec_address_set_cast(ovsdb_idl_cursor_data(cursor));
}
...
nbrec_address_set_index_set_addresses(const struct nbrec_address_set *row, const char **addresses, size_t n_addresses)
{
    ...
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_STRING, OVSDB_TYPE_VOID);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_address_set_columns[ NBREC_ADDRESS_SET_COL_ADDRESSES ], &datum, &nbrec_table_classes[NBREC_TABLE_ADDRESS_SET]);
}
...
nbrec_address_set_index_set_name(const struct nbrec_address_set *row, const char *name)
{
    ...
    key->string = CONST_CAST(char *, name);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_address_set_columns[ NBREC_ADDRESS_SET_COL_NAME ], &datum, &nbrec_table_classes[NBREC_TABLE_ADDRESS_SET]);
}
...
const struct nbrec_connection_table *
nbrec_connection_table_get(const struct ovsdb_idl *idl)
{
    return (const struct nbrec_connection_table *) idl;
}

const struct nbrec_connection *
nbrec_connection_table_first(const struct nbrec_connection_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_connection_first(idl);
}


const struct nbrec_connection *
nbrec_connection_table_track_get_first(const struct nbrec_connection_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return nbrec_connection_track_get_first(idl);
}
 ...
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct nbrec_connection *
nbrec_connection_table_get_for_uuid(const struct nbrec_connection_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_connection_get_for_uuid(idl, uuid);
}
...
struct nbrec_connection *
nbrec_connection_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_connection);
    return ALIGNED_CAST(struct nbrec_connection *, ovsdb_idl_index_init_row(index));
}

struct nbrec_connection *
nbrec_connection_index_find(struct ovsdb_idl_index *index, const struct nbrec_connection *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_connection);
    return nbrec_connection_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
nbrec_connection_index_compare(
    struct ovsdb_idl_index *index, 
    const struct nbrec_connection *a, 
    const struct nbrec_connection *b)
{
    return ovsdb_idl_index_compare(index, &a->header_, &b->header_);
}

struct ovsdb_idl_cursor
nbrec_connection_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_connection);
    return ovsdb_idl_cursor_first(index);
}
...
struct ovsdb_idl_cursor
nbrec_connection_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct nbrec_connection *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_connection);
    return ovsdb_idl_cursor_first_eq(index, &target->header_);
}
...
struct ovsdb_idl_cursor
nbrec_connection_cursor_first_ge(
    struct ovsdb_idl_index *index, const struct nbrec_connection *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_connection);
    return ovsdb_idl_cursor_first_ge(index, &target->header_);
...
struct nbrec_connection *
nbrec_connection_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return nbrec_connection_cast(ovsdb_idl_cursor_data(cursor));
}
...
nbrec_connection_index_set_inactivity_probe(const struct nbrec_connection *row, const int64_t *inactivity_probe, size_t n_inactivity_probe)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_connection_columns[ NBREC_CONNECTION_COL_INACTIVITY_PROBE ], &datum, &nbrec_table_classes[NBREC_TABLE_CONNECTION]);
}
...
nbrec_connection_index_set_is_connected(const struct nbrec_connection *row, bool is_connected)
{
    ...
    key->boolean = is_connected;
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_connection_columns[ NBREC_CONNECTION_COL_IS_CONNECTED ], &datum, &nbrec_table_classes[NBREC_TABLE_CONNECTION]);
}
...
nbrec_connection_index_set_max_backoff(const struct nbrec_connection *row, const int64_t *max_backoff, size_t n_max_backoff)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_connection_columns[ NBREC_CONNECTION_COL_MAX_BACKOFF ], &datum, &nbrec_table_classes[NBREC_TABLE_CONNECTION]);
}
...
nbrec_connection_index_set_target(const struct nbrec_connection *row, const char *target)
{
    ...
    key->string = CONST_CAST(char *, target);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_connection_columns[ NBREC_CONNECTION_COL_TARGET ], &datum, &nbrec_table_classes[NBREC_TABLE_CONNECTION]);
}
...
const struct nbrec_dhcp_options_table *
nbrec_dhcp_options_table_get(const struct ovsdb_idl *idl)
{
    return (const struct nbrec_dhcp_options_table *) idl;
}

const struct nbrec_dhcp_options *
nbrec_dhcp_options_table_first(const struct nbrec_dhcp_options_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_dhcp_options_first(idl);
}


const struct nbrec_dhcp_options *
nbrec_dhcp_options_table_track_get_first(const struct nbrec_dhcp_options_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return nbrec_dhcp_options_track_get_first(idl);
}
 ...
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct nbrec_dhcp_options *
nbrec_dhcp_options_table_get_for_uuid(const struct nbrec_dhcp_options_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_dhcp_options_get_for_uuid(idl, uuid);
}
...
struct nbrec_dhcp_options *
nbrec_dhcp_options_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_dhcp_options);
    return ALIGNED_CAST(struct nbrec_dhcp_options *, ovsdb_idl_index_init_row(index));
}

struct nbrec_dhcp_options *
nbrec_dhcp_options_index_find(struct ovsdb_idl_index *index, const struct nbrec_dhcp_options *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_dhcp_options);
    return nbrec_dhcp_options_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
nbrec_dhcp_options_index_compare(
    struct ovsdb_idl_index *index, 
    const struct nbrec_dhcp_options *a, 
    const struct nbrec_dhcp_options *b)
{
    return ovsdb_idl_index_compare(index, &a->header_, &b->header_);
}

struct ovsdb_idl_cursor
nbrec_dhcp_options_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_dhcp_options);
    return ovsdb_idl_cursor_first(index);
}
...
struct ovsdb_idl_cursor
nbrec_dhcp_options_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct nbrec_dhcp_options *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_dhcp_options);
    return ovsdb_idl_cursor_first_eq(index, &target->header_);
}
...
struct ovsdb_idl_cursor
nbrec_dhcp_options_cursor_first_ge(
    struct ovsdb_idl_index *index, const struct nbrec_dhcp_options *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_dhcp_options);
    return ovsdb_idl_cursor_first_ge(index, &target->header_);
...
struct nbrec_dhcp_options *
nbrec_dhcp_options_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return nbrec_dhcp_options_cast(ovsdb_idl_cursor_data(cursor));
}
...
nbrec_dhcp_options_index_set_cidr(const struct nbrec_dhcp_options *row, const char *cidr)
{
    ...
    key->string = CONST_CAST(char *, cidr);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_dhcp_options_columns[ NBREC_DHCP_OPTIONS_COL_CIDR ], &datum, &nbrec_table_classes[NBREC_TABLE_DHCP_OPTIONS]);
}
...
const struct nbrec_dns_table *
nbrec_dns_table_get(const struct ovsdb_idl *idl)
{
    return (const struct nbrec_dns_table *) idl;
}

const struct nbrec_dns *
nbrec_dns_table_first(const struct nbrec_dns_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_dns_first(idl);
}


const struct nbrec_dns *
nbrec_dns_table_track_get_first(const struct nbrec_dns_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return nbrec_dns_track_get_first(idl);
}
 ...
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct nbrec_dns *
nbrec_dns_table_get_for_uuid(const struct nbrec_dns_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_dns_get_for_uuid(idl, uuid);
}
...
struct nbrec_dns *
nbrec_dns_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_dns);
    return ALIGNED_CAST(struct nbrec_dns *, ovsdb_idl_index_init_row(index));
}

struct nbrec_dns *
nbrec_dns_index_find(struct ovsdb_idl_index *index, const struct nbrec_dns *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_dns);
    return nbrec_dns_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
nbrec_dns_index_compare(
    struct ovsdb_idl_index *index, 
    const struct nbrec_dns *a, 
    const struct nbrec_dns *b)
{
    return ovsdb_idl_index_compare(index, &a->header_, &b->header_);
}

struct ovsdb_idl_cursor
nbrec_dns_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_dns);
    return ovsdb_idl_cursor_first(index);
}
...
struct ovsdb_idl_cursor
nbrec_dns_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct nbrec_dns *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_dns);
    return ovsdb_idl_cursor_first_eq(index, &target->header_);
}
...
struct nbrec_dns *
nbrec_dns_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return nbrec_dns_cast(ovsdb_idl_cursor_data(cursor));
}
...
const struct nbrec_gateway_chassis_table *
nbrec_gateway_chassis_table_get(const struct ovsdb_idl *idl)
{
    return (const struct nbrec_gateway_chassis_table *) idl;
}

const struct nbrec_gateway_chassis *
nbrec_gateway_chassis_table_first(const struct nbrec_gateway_chassis_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_gateway_chassis_first(idl);
}


const struct nbrec_gateway_chassis *
nbrec_gateway_chassis_table_track_get_first(const struct nbrec_gateway_chassis_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return nbrec_gateway_chassis_track_get_first(idl);
}
 ...
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct nbrec_gateway_chassis *
nbrec_gateway_chassis_table_get_for_uuid(const struct nbrec_gateway_chassis_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_gateway_chassis_get_for_uuid(idl, uuid);
}
...
struct nbrec_gateway_chassis *
nbrec_gateway_chassis_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_gateway_chassis);
    return ALIGNED_CAST(struct nbrec_gateway_chassis *, ovsdb_idl_index_init_row(index));
}

struct nbrec_gateway_chassis *
nbrec_gateway_chassis_index_find(struct ovsdb_idl_index *index, const struct nbrec_gateway_chassis *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_gateway_chassis);
    return nbrec_gateway_chassis_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
nbrec_gateway_chassis_index_compare(
    struct ovsdb_idl_index *index, 
    const struct nbrec_gateway_chassis *a, 
    const struct nbrec_gateway_chassis *b)
{
    return ovsdb_idl_index_compare(index, &a->header_, &b->header_);
}

struct ovsdb_idl_cursor
nbrec_gateway_chassis_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_gateway_chassis);
    return ovsdb_idl_cursor_first(index);
}
...
struct ovsdb_idl_cursor
nbrec_gateway_chassis_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct nbrec_gateway_chassis *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_gateway_chassis);
    return ovsdb_idl_cursor_first_eq(index, &target->header_);
}
...
struct ovsdb_idl_cursor
nbrec_gateway_chassis_cursor_first_ge(
    struct ovsdb_idl_index *index, const struct nbrec_gateway_chassis *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_gateway_chassis);
    return ovsdb_idl_cursor_first_ge(index, &target->header_);
...
struct nbrec_gateway_chassis *
nbrec_gateway_chassis_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return nbrec_gateway_chassis_cast(ovsdb_idl_cursor_data(cursor));
}
...
nbrec_gateway_chassis_index_set_chassis_name(const struct nbrec_gateway_chassis *row, const char *chassis_name)
{
    ...
    key->string = CONST_CAST(char *, chassis_name);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_gateway_chassis_columns[ NBREC_GATEWAY_CHASSIS_COL_CHASSIS_NAME ], &datum, &nbrec_table_classes[NBREC_TABLE_GATEWAY_CHASSIS]);
}
...
nbrec_gateway_chassis_index_set_name(const struct nbrec_gateway_chassis *row, const char *name)
{
    ...
    key->string = CONST_CAST(char *, name);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_gateway_chassis_columns[ NBREC_GATEWAY_CHASSIS_COL_NAME ], &datum, &nbrec_table_classes[NBREC_TABLE_GATEWAY_CHASSIS]);
}
...
nbrec_gateway_chassis_index_set_priority(const struct nbrec_gateway_chassis *row, int64_t priority)
{
    ...
    key->integer = priority;
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_gateway_chassis_columns[ NBREC_GATEWAY_CHASSIS_COL_PRIORITY ], &datum, &nbrec_table_classes[NBREC_TABLE_GATEWAY_CHASSIS]);
}
...
nbrec_load_balancer_table_get(const struct ovsdb_idl *idl)
{
    return (const struct nbrec_load_balancer_table *) idl;
}

const struct nbrec_load_balancer *
nbrec_load_balancer_table_first(const struct nbrec_load_balancer_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_load_balancer_first(idl);
}


const struct nbrec_load_balancer *
nbrec_load_balancer_table_track_get_first(const struct nbrec_load_balancer_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return nbrec_load_balancer_track_get_first(idl);
}
...
/* Searches table "Load_Balancer" for a row with UUID 'uuid'.  Returns
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct nbrec_load_balancer *
nbrec_load_balancer_table_get_for_uuid(const struct nbrec_load_balancer_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_load_balancer_get_for_uuid(idl, uuid);
}
...
struct nbrec_load_balancer *
nbrec_load_balancer_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_load_balancer);
    return ALIGNED_CAST(struct nbrec_load_balancer *, ovsdb_idl_index_init_row(index));
}

struct nbrec_load_balancer *
nbrec_load_balancer_index_find(struct ovsdb_idl_index *index, const struct nbrec_load_balancer *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_load_balancer);
    return nbrec_load_balancer_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
nbrec_load_balancer_index_compare(
    struct ovsdb_idl_index *index, 
    const struct nbrec_load_balancer *a, 
    const struct nbrec_load_balancer *b)
{
    return ovsdb_idl_index_compare(index, &a->header_, &b->header_);
}

struct ovsdb_idl_cursor
nbrec_load_balancer_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_load_balancer);
    return ovsdb_idl_cursor_first(index);
}
...
struct ovsdb_idl_cursor
nbrec_load_balancer_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct nbrec_load_balancer *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_load_balancer);
    return ovsdb_idl_cursor_first_eq(index, &target->header_);
}
...
struct ovsdb_idl_cursor
nbrec_load_balancer_cursor_first_ge(
    struct ovsdb_idl_index *index, const struct nbrec_load_balancer *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_load_balancer);
    return ovsdb_idl_cursor_first_ge(index, &target->header_);
...
struct nbrec_load_balancer *
nbrec_load_balancer_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return nbrec_load_balancer_cast(ovsdb_idl_cursor_data(cursor));
}
...
nbrec_load_balancer_index_set_name(const struct nbrec_load_balancer *row, const char *name)
{
    ...
    key->string = CONST_CAST(char *, name);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_load_balancer_columns[ NBREC_LOAD_BALANCER_COL_NAME ], &datum, &nbrec_table_classes[NBREC_TABLE_LOAD_BALANCER]);
}
...
nbrec_load_balancer_index_set_protocol(const struct nbrec_load_balancer *row, const char *protocol)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_load_balancer_columns[ NBREC_LOAD_BALANCER_COL_PROTOCOL ], &datum, &nbrec_table_classes[NBREC_TABLE_LOAD_BALANCER]);
}
...
nbrec_logical_router_table_get(const struct ovsdb_idl *idl)
{
    return (const struct nbrec_logical_router_table *) idl;
}

const struct nbrec_logical_router *
nbrec_logical_router_table_first(const struct nbrec_logical_router_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_logical_router_first(idl);
}


const struct nbrec_logical_router *
nbrec_logical_router_table_track_get_first(const struct nbrec_logical_router_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return nbrec_logical_router_track_get_first(idl);
}
...
/* Searches table "Logical_Router" for a row with UUID 'uuid'.  Returns
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct nbrec_logical_router *
nbrec_logical_router_table_get_for_uuid(const struct nbrec_logical_router_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_logical_router_get_for_uuid(idl, uuid);
}
...
struct nbrec_logical_router *
nbrec_logical_router_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_router);
    return ALIGNED_CAST(struct nbrec_logical_router *, ovsdb_idl_index_init_row(index));
}

struct nbrec_logical_router *
nbrec_logical_router_index_find(struct ovsdb_idl_index *index, const struct nbrec_logical_router *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_router);
    return nbrec_logical_router_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
nbrec_logical_router_index_compare(
    struct ovsdb_idl_index *index, 
    const struct nbrec_logical_router *a, 
    const struct nbrec_logical_router *b)
{
    return ovsdb_idl_index_compare(index, &a->header_, &b->header_);
}

struct ovsdb_idl_cursor
nbrec_logical_router_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_router);
    return ovsdb_idl_cursor_first(index);
}
...
struct ovsdb_idl_cursor
nbrec_logical_router_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct nbrec_logical_router *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_router);
    return ovsdb_idl_cursor_first_eq(index, &target->header_);
}
...
struct ovsdb_idl_cursor
nbrec_logical_router_cursor_first_ge(
    struct ovsdb_idl_index *index, const struct nbrec_logical_router *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_router);
    return ovsdb_idl_cursor_first_ge(index, &target->header_);
...
struct nbrec_logical_router *
nbrec_logical_router_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return nbrec_logical_router_cast(ovsdb_idl_cursor_data(cursor));
}
...
nbrec_logical_router_index_set_enabled(const struct nbrec_logical_router *row, const bool *enabled, size_t n_enabled)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_router_columns[ NBREC_LOGICAL_ROUTER_COL_ENABLED ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_ROUTER]);
}
...
nbrec_logical_router_index_set_load_balancer(const struct nbrec_logical_router *row, struct nbrec_load_balancer **load_balancer, size_t n_load_balancer)
{
    ...
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_UUID, OVSDB_TYPE_VOID);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_router_columns[ NBREC_LOGICAL_ROUTER_COL_LOAD_BALANCER ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_ROUTER]);
}
...
nbrec_logical_router_index_set_name(const struct nbrec_logical_router *row, const char *name)
{
    ...
    key->string = CONST_CAST(char *, name);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_router_columns[ NBREC_LOGICAL_ROUTER_COL_NAME ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_ROUTER]);
}
...
nbrec_logical_router_index_set_nat(const struct nbrec_logical_router *row, struct nbrec_nat **nat, size_t n_nat)
{
    ...
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_UUID, OVSDB_TYPE_VOID);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_router_columns[ NBREC_LOGICAL_ROUTER_COL_NAT ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_ROUTER]);
}
...
nbrec_logical_router_index_set_ports(const struct nbrec_logical_router *row, struct nbrec_logical_router_port **ports, size_t n_ports)
{
    ...
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_UUID, OVSDB_TYPE_VOID);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_router_columns[ NBREC_LOGICAL_ROUTER_COL_PORTS ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_ROUTER]);
}
...
nbrec_logical_router_index_set_static_routes(const struct nbrec_logical_router *row, struct nbrec_logical_router_static_route **static_routes, size_t n_static_routes)
{
    ...
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_UUID, OVSDB_TYPE_VOID);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_router_columns[ NBREC_LOGICAL_ROUTER_COL_STATIC_ROUTES ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_ROUTER]);
}
...
struct ovsdb_idl_column nbrec_logical_router_columns[NBREC_LOGICAL_ROUTER_N_COLUMNS] = {
    ...
    [NBREC_LOGICAL_ROUTER_COL_LOAD_BALANCER] = {
         ...
         .type = {
            .key = {
               ...
               .uuid = { .refTableName = "Load_Balancer", .refType = OVSDB_REF_WEAK },
            },
            .value = OVSDB_BASE_VOID_INIT,
            .n_min = 0,
            .n_max = UINT_MAX,
    ...
    [NBREC_LOGICAL_ROUTER_COL_NAT] = {
         ...
         .type = {
            .key = {
               .type = OVSDB_TYPE_UUID,
               .uuid = { .refTableName = "NAT", .refType = OVSDB_REF_STRONG },
            },
            .value = OVSDB_BASE_VOID_INIT,
            .n_min = 0,
            .n_max = UINT_MAX,
    ...
    [NBREC_LOGICAL_ROUTER_COL_PORTS] = {
         ...
         .type = {
            .key = {
               .type = OVSDB_TYPE_UUID,
               .uuid = { .refTableName = "Logical_Router_Port", .refType = OVSDB_REF_STRONG },
            },
            .value = OVSDB_BASE_VOID_INIT,
            .n_min = 0,
            .n_max = UINT_MAX,
    ...
    [NBREC_LOGICAL_ROUTER_COL_STATIC_ROUTES] = {
         ...
         .type = {
            .key = {
               .type = OVSDB_TYPE_UUID,
               .uuid = { .refTableName = "Logical_Router_Static_Route", .refType = OVSDB_REF_STRONG },
            },
            .value = OVSDB_BASE_VOID_INIT,
            .n_min = 0,
            .n_max = UINT_MAX,
...
nbrec_logical_router_port_table_get(const struct ovsdb_idl *idl)
{
    return (const struct nbrec_logical_router_port_table *) idl;
}

const struct nbrec_logical_router_port *
nbrec_logical_router_port_table_first(const struct nbrec_logical_router_port_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_logical_router_port_first(idl);
}


const struct nbrec_logical_router_port *
nbrec_logical_router_port_table_track_get_first(const struct nbrec_logical_router_port_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return nbrec_logical_router_port_track_get_first(idl);
}
...
static void
nbrec_logical_router_port_parse_ipv6_ra_configs(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct nbrec_logical_router_port *row = nbrec_logical_router_port_cast(row_);
    smap_init(&row->ipv6_ra_configs);
    for (size_t i = 0; i < datum->n; i++) {
        smap_add(&row->ipv6_ra_configs,
                 datum->keys[i].string,
                 datum->values[i].string);
    }
}
...
static void
nbrec_logical_router_port_unparse_ipv6_ra_configs(struct ovsdb_idl_row *row_)
{
    struct nbrec_logical_router_port *row = nbrec_logical_router_port_cast(row_);
    smap_destroy(&row->ipv6_ra_configs);
}
...
/* Searches table "Logical_Router_Port" for a row with UUID 'uuid'.  Returns
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct nbrec_logical_router_port *
nbrec_logical_router_port_table_get_for_uuid(const struct nbrec_logical_router_port_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_logical_router_port_get_for_uuid(idl, uuid);
}
...
/* Causes the original contents of column "ipv6_ra_configs" in 'row' to be
 * verified as a prerequisite to completing the transaction.  That is, if
 * "ipv6_ra_configs" in 'row' changed (or if 'row' was deleted) between the
 * time that the IDL originally read its contents and the time that the
 * transaction aborts and ovsdb_idl_txn_commit() returns TXN_TRY_AGAIN.
 *
 * The intention is that, to ensure that no transaction commits based on dirty
 * reads, an application should call this function any time "ipv6_ra_configs" is
 * read as part of a read-modify-write operation.
 *
 * In some cases this function reduces to a no-op, because the current value
 * of "ipv6_ra_configs" is already known:
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     nbrec_logical_router_port_insert()).
 *
 *   - If "ipv6_ra_configs" has already been modified (with
 *     nbrec_logical_router_port_set_ipv6_ra_configs()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * nbrec_logical_router_port_set_ipv6_ra_configs() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
nbrec_logical_router_port_verify_ipv6_ra_configs(const struct nbrec_logical_router_port *row)
{
    ovsdb_idl_txn_verify(&row->header_, &nbrec_logical_router_port_col_ipv6_ra_configs);
}
...
/* Returns the "ipv6_ra_configs" column's value from the "Logical_Router_Port" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 ...
 * 'key_type' must be OVSDB_TYPE_STRING.
 * 'value_type' must be OVSDB_TYPE_STRING.
 * (This helps to avoid silent bugs if someone changes ipv6_ra_configs's
 * type without updating the caller.)
 *
 * The caller must not modify or free the returned value.
 *
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "ipv6_ra_configs" member in nbrec_logical_router_port. */
const struct ovsdb_datum *
nbrec_logical_router_port_get_ipv6_ra_configs(const struct nbrec_logical_router_port *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED,
	enum ovsdb_atomic_type value_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    ovs_assert(value_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &nbrec_logical_router_port_col_ipv6_ra_configs);
}
...
/* Sets the "ipv6_ra_configs" column's value from the "Logical_Router_Port" table in 'row'
 * to 'ipv6_ra_configs'.
 *
 * The caller retains ownership of 'ipv6_ra_configs' and everything in it. */
void
nbrec_logical_router_port_set_ipv6_ra_configs(const struct nbrec_logical_router_port *row, const struct smap *ipv6_ra_configs)
{
    struct ovsdb_datum datum;

    if (ipv6_ra_configs) {
        ovsdb_datum_from_smap(&datum, ipv6_ra_configs);
    } else {
        ovsdb_datum_init_empty(&datum);
    }
    ovsdb_idl_txn_write(&row->header_,
                        &nbrec_logical_router_port_col_ipv6_ra_configs,
                        &datum);
}
...
/* Sets an element of the "ipv6_ra_configs" map column from the "Logical_Router_Port" table in 'row'
 * to 'new_value' given the key value 'new_key'.
 *
 */
void
nbrec_logical_router_port_update_ipv6_ra_configs_setkey(const struct nbrec_logical_router_port *row, const char *new_key, const char *new_value)
{
    struct ovsdb_datum *datum;
...
/* Deletes an element of the "ipv6_ra_configs" map column from the "Logical_Router_Port" table in 'row'
 * given the key value 'delete_key'.
 *
 */
void
nbrec_logical_router_port_update_ipv6_ra_configs_delkey(const struct nbrec_logical_router_port *row, const char *delete_key)
{
    struct ovsdb_datum *datum;
 ...
 * The caller retains ownership of 'ipv6_ra_configs' and everything in it. */
void
nbrec_logical_router_port_add_clause_ipv6_ra_configs(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const struct smap *ipv6_ra_configs)
{
    struct ovsdb_datum datum;
    ...
    ovsdb_datum_destroy(&datum, &nbrec_logical_router_port_col_ipv6_ra_configs.type);
}
...
struct nbrec_logical_router_port *
nbrec_logical_router_port_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_router_port);
    return ALIGNED_CAST(struct nbrec_logical_router_port *, ovsdb_idl_index_init_row(index));
}

struct nbrec_logical_router_port *
nbrec_logical_router_port_index_find(struct ovsdb_idl_index *index, const struct nbrec_logical_router_port *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_router_port);
    return nbrec_logical_router_port_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
nbrec_logical_router_port_index_compare(
    struct ovsdb_idl_index *index, 
    const struct nbrec_logical_router_port *a, 
    const struct nbrec_logical_router_port *b)
{
    return ovsdb_idl_index_compare(index, &a->header_, &b->header_);
}

struct ovsdb_idl_cursor
nbrec_logical_router_port_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_router_port);
    return ovsdb_idl_cursor_first(index);
}
...
struct ovsdb_idl_cursor
nbrec_logical_router_port_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct nbrec_logical_router_port *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_router_port);
    return ovsdb_idl_cursor_first_eq(index, &target->header_);
}
...
struct ovsdb_idl_cursor
nbrec_logical_router_port_cursor_first_ge(
    struct ovsdb_idl_index *index, const struct nbrec_logical_router_port *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_router_port);
    return ovsdb_idl_cursor_first_ge(index, &target->header_);
...
struct nbrec_logical_router_port *
nbrec_logical_router_port_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return nbrec_logical_router_port_cast(ovsdb_idl_cursor_data(cursor));
}
...
nbrec_logical_router_port_index_set_enabled(const struct nbrec_logical_router_port *row, const bool *enabled, size_t n_enabled)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_router_port_columns[ NBREC_LOGICAL_ROUTER_PORT_COL_ENABLED ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_ROUTER_PORT]);
}
...
nbrec_logical_router_port_index_set_gateway_chassis(const struct nbrec_logical_router_port *row, struct nbrec_gateway_chassis **gateway_chassis, size_t n_gateway_chassis)
{
    ...
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_UUID, OVSDB_TYPE_VOID);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_router_port_columns[ NBREC_LOGICAL_ROUTER_PORT_COL_GATEWAY_CHASSIS ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_ROUTER_PORT]);
}
 ...
 * The caller retains ownership of 'ipv6_ra_configs' and everything in it. */
void
nbrec_logical_router_port_index_set_ipv6_ra_configs(const struct nbrec_logical_router_port *row, const struct smap *ipv6_ra_configs)
{
    struct ovsdb_datum *datum = xmalloc(sizeof(struct ovsdb_datum));
    ...
    if (ipv6_ra_configs) {
        ...
        datum->n = smap_count(ipv6_ra_configs);
        datum->keys = xmalloc(datum->n * sizeof *datum->keys);
        datum->values = xmalloc(datum->n * sizeof *datum->values);

        i = 0;
        SMAP_FOR_EACH (node, ipv6_ra_configs) {
            datum->keys[i].string = node->key;
            datum->values[i].string = node->value;
            i++;
        }
    ...
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_),
                          &nbrec_logical_router_port_columns[NBREC_LOGICAL_ROUTER_PORT_COL_IPV6_RA_CONFIGS],
                          datum,
                          &nbrec_table_classes[NBREC_TABLE_LOGICAL_ROUTER_PORT]);
}
...
nbrec_logical_router_port_index_set_mac(const struct nbrec_logical_router_port *row, const char *mac)
{
    ...
    key->string = CONST_CAST(char *, mac);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_router_port_columns[ NBREC_LOGICAL_ROUTER_PORT_COL_MAC ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_ROUTER_PORT]);
}
...
nbrec_logical_router_port_index_set_name(const struct nbrec_logical_router_port *row, const char *name)
{
    ...
    key->string = CONST_CAST(char *, name);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_router_port_columns[ NBREC_LOGICAL_ROUTER_PORT_COL_NAME ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_ROUTER_PORT]);
}
...
nbrec_logical_router_port_index_set_networks(const struct nbrec_logical_router_port *row, const char **networks, size_t n_networks)
{
    ...
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_STRING, OVSDB_TYPE_VOID);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_router_port_columns[ NBREC_LOGICAL_ROUTER_PORT_COL_NETWORKS ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_ROUTER_PORT]);
}
...
nbrec_logical_router_port_index_set_peer(const struct nbrec_logical_router_port *row, const char *peer)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_router_port_columns[ NBREC_LOGICAL_ROUTER_PORT_COL_PEER ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_ROUTER_PORT]);
}
...
struct ovsdb_idl_column nbrec_logical_router_port_columns[NBREC_LOGICAL_ROUTER_PORT_N_COLUMNS] = {
    ...
    [NBREC_LOGICAL_ROUTER_PORT_COL_IPV6_RA_CONFIGS] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = nbrec_logical_router_port_parse_ipv6_ra_configs,
         .unparse = nbrec_logical_router_port_unparse_ipv6_ra_configs,
    },
...
nbrec_logical_router_static_route_table_get(const struct ovsdb_idl *idl)
{
    return (const struct nbrec_logical_router_static_route_table *) idl;
}

const struct nbrec_logical_router_static_route *
nbrec_logical_router_static_route_table_first(const struct nbrec_logical_router_static_route_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_logical_router_static_route_first(idl);
}


const struct nbrec_logical_router_static_route *
nbrec_logical_router_static_route_table_track_get_first(const struct nbrec_logical_router_static_route_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return nbrec_logical_router_static_route_track_get_first(idl);
}


static void
nbrec_logical_router_static_route_parse_external_ids(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
...
static void
nbrec_logical_router_static_route_unparse_external_ids(struct ovsdb_idl_row *row_)
{
    struct nbrec_logical_router_static_route *row = nbrec_logical_router_static_route_cast(row_);
    smap_destroy(&row->external_ids);
}
...
/* Searches table "Logical_Router_Static_Route" for a row with UUID 'uuid'.  Returns
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct nbrec_logical_router_static_route *
nbrec_logical_router_static_route_table_get_for_uuid(const struct nbrec_logical_router_static_route_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_logical_router_static_route_get_for_uuid(idl, uuid);
}
 ...
 *
 *   - If "external_ids" has already been modified (with
 *     nbrec_logical_router_static_route_set_external_ids()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * nbrec_logical_router_static_route_set_external_ids() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
nbrec_logical_router_static_route_verify_external_ids(const struct nbrec_logical_router_static_route *row)
{
    ovsdb_idl_txn_verify(&row->header_, &nbrec_logical_router_static_route_col_external_ids);
}
...
/* Returns the "external_ids" column's value from the "Logical_Router_Static_Route" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "external_ids" member in nbrec_logical_router_static_route. */
const struct ovsdb_datum *
nbrec_logical_router_static_route_get_external_ids(const struct nbrec_logical_router_static_route *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED,
	enum ovsdb_atomic_type value_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    ovs_assert(value_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &nbrec_logical_router_static_route_col_external_ids);
}
...
/* Sets the "external_ids" column's value from the "Logical_Router_Static_Route" table in 'row'
 * to 'external_ids'.
 *
 * The caller retains ownership of 'external_ids' and everything in it. */
void
nbrec_logical_router_static_route_set_external_ids(const struct nbrec_logical_router_static_route *row, const struct smap *external_ids)
{
    struct ovsdb_datum datum;
...
/* Sets an element of the "external_ids" map column from the "Logical_Router_Static_Route" table in 'row'
 * to 'new_value' given the key value 'new_key'.
 *
 */
void
nbrec_logical_router_static_route_update_external_ids_setkey(const struct nbrec_logical_router_static_route *row, const char *new_key, const char *new_value)
{
    struct ovsdb_datum *datum;
...
/* Deletes an element of the "external_ids" map column from the "Logical_Router_Static_Route" table in 'row'
 * given the key value 'delete_key'.
 *
 */
void
nbrec_logical_router_static_route_update_external_ids_delkey(const struct nbrec_logical_router_static_route *row, const char *delete_key)
{
    struct ovsdb_datum *datum;
 ...
 * The caller retains ownership of 'external_ids' and everything in it. */
void
nbrec_logical_router_static_route_add_clause_external_ids(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const struct smap *external_ids)
{
    struct ovsdb_datum datum;
    ...
    ovsdb_datum_destroy(&datum, &nbrec_logical_router_static_route_col_external_ids.type);
}
...
struct nbrec_logical_router_static_route *
nbrec_logical_router_static_route_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_router_static_route);
    return ALIGNED_CAST(struct nbrec_logical_router_static_route *, ovsdb_idl_index_init_row(index));
}

struct nbrec_logical_router_static_route *
nbrec_logical_router_static_route_index_find(struct ovsdb_idl_index *index, const struct nbrec_logical_router_static_route *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_router_static_route);
    return nbrec_logical_router_static_route_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
nbrec_logical_router_static_route_index_compare(
    struct ovsdb_idl_index *index, 
    const struct nbrec_logical_router_static_route *a, 
    const struct nbrec_logical_router_static_route *b)
{
    return ovsdb_idl_index_compare(index, &a->header_, &b->header_);
}

struct ovsdb_idl_cursor
nbrec_logical_router_static_route_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_router_static_route);
    return ovsdb_idl_cursor_first(index);
}
...
struct ovsdb_idl_cursor
nbrec_logical_router_static_route_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct nbrec_logical_router_static_route *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_router_static_route);
    return ovsdb_idl_cursor_first_eq(index, &target->header_);
}
...
struct ovsdb_idl_cursor
nbrec_logical_router_static_route_cursor_first_ge(
    struct ovsdb_idl_index *index, const struct nbrec_logical_router_static_route *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_router_static_route);
    return ovsdb_idl_cursor_first_ge(index, &target->header_);
...
struct nbrec_logical_router_static_route *
nbrec_logical_router_static_route_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return nbrec_logical_router_static_route_cast(ovsdb_idl_cursor_data(cursor));
}
 ...
 * The caller retains ownership of 'external_ids' and everything in it. */
void
nbrec_logical_router_static_route_index_set_external_ids(const struct nbrec_logical_router_static_route *row, const struct smap *external_ids)
{
    struct ovsdb_datum *datum = xmalloc(sizeof(struct ovsdb_datum));
    ...
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_),
                          &nbrec_logical_router_static_route_columns[NBREC_LOGICAL_ROUTER_STATIC_ROUTE_COL_EXTERNAL_IDS],
                          datum,
                          &nbrec_table_classes[NBREC_TABLE_LOGICAL_ROUTER_STATIC_ROUTE]);
}
...
nbrec_logical_router_static_route_index_set_ip_prefix(const struct nbrec_logical_router_static_route *row, const char *ip_prefix)
{
    ...
    key->string = CONST_CAST(char *, ip_prefix);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_router_static_route_columns[ NBREC_LOGICAL_ROUTER_STATIC_ROUTE_COL_IP_PREFIX ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_ROUTER_STATIC_ROUTE]);
}
...
nbrec_logical_router_static_route_index_set_nexthop(const struct nbrec_logical_router_static_route *row, const char *nexthop)
{
    ...
    key->string = CONST_CAST(char *, nexthop);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_router_static_route_columns[ NBREC_LOGICAL_ROUTER_STATIC_ROUTE_COL_NEXTHOP ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_ROUTER_STATIC_ROUTE]);
}
...
nbrec_logical_router_static_route_index_set_output_port(const struct nbrec_logical_router_static_route *row, const char *output_port)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_router_static_route_columns[ NBREC_LOGICAL_ROUTER_STATIC_ROUTE_COL_OUTPUT_PORT ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_ROUTER_STATIC_ROUTE]);
}
...
nbrec_logical_router_static_route_index_set_policy(const struct nbrec_logical_router_static_route *row, const char *policy)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_router_static_route_columns[ NBREC_LOGICAL_ROUTER_STATIC_ROUTE_COL_POLICY ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_ROUTER_STATIC_ROUTE]);
}
...
struct ovsdb_idl_column nbrec_logical_router_static_route_columns[NBREC_LOGICAL_ROUTER_STATIC_ROUTE_N_COLUMNS] = {
    [NBREC_LOGICAL_ROUTER_STATIC_ROUTE_COL_EXTERNAL_IDS] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = nbrec_logical_router_static_route_parse_external_ids,
         .unparse = nbrec_logical_router_static_route_unparse_external_ids,
    },
...
nbrec_logical_switch_table_get(const struct ovsdb_idl *idl)
{
    return (const struct nbrec_logical_switch_table *) idl;
}

const struct nbrec_logical_switch *
nbrec_logical_switch_table_first(const struct nbrec_logical_switch_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_logical_switch_first(idl);
}


const struct nbrec_logical_switch *
nbrec_logical_switch_table_track_get_first(const struct nbrec_logical_switch_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return nbrec_logical_switch_track_get_first(idl);
}
...
/* Searches table "Logical_Switch" for a row with UUID 'uuid'.  Returns
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct nbrec_logical_switch *
nbrec_logical_switch_table_get_for_uuid(const struct nbrec_logical_switch_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_logical_switch_get_for_uuid(idl, uuid);
}
...
struct nbrec_logical_switch *
nbrec_logical_switch_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_switch);
    return ALIGNED_CAST(struct nbrec_logical_switch *, ovsdb_idl_index_init_row(index));
}

struct nbrec_logical_switch *
nbrec_logical_switch_index_find(struct ovsdb_idl_index *index, const struct nbrec_logical_switch *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_switch);
    return nbrec_logical_switch_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
nbrec_logical_switch_index_compare(
    struct ovsdb_idl_index *index, 
    const struct nbrec_logical_switch *a, 
    const struct nbrec_logical_switch *b)
{
    return ovsdb_idl_index_compare(index, &a->header_, &b->header_);
}

struct ovsdb_idl_cursor
nbrec_logical_switch_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_switch);
    return ovsdb_idl_cursor_first(index);
}
...
struct ovsdb_idl_cursor
nbrec_logical_switch_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct nbrec_logical_switch *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_switch);
    return ovsdb_idl_cursor_first_eq(index, &target->header_);
}
...
struct ovsdb_idl_cursor
nbrec_logical_switch_cursor_first_ge(
    struct ovsdb_idl_index *index, const struct nbrec_logical_switch *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_switch);
    return ovsdb_idl_cursor_first_ge(index, &target->header_);
...
struct nbrec_logical_switch *
nbrec_logical_switch_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return nbrec_logical_switch_cast(ovsdb_idl_cursor_data(cursor));
}
...
nbrec_logical_switch_index_set_acls(const struct nbrec_logical_switch *row, struct nbrec_acl **acls, size_t n_acls)
{
    ...
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_UUID, OVSDB_TYPE_VOID);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_switch_columns[ NBREC_LOGICAL_SWITCH_COL_ACLS ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_SWITCH]);
}
...
nbrec_logical_switch_index_set_dns_records(const struct nbrec_logical_switch *row, struct nbrec_dns **dns_records, size_t n_dns_records)
{
    ...
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_UUID, OVSDB_TYPE_VOID);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_switch_columns[ NBREC_LOGICAL_SWITCH_COL_DNS_RECORDS ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_SWITCH]);
}
...
nbrec_logical_switch_index_set_load_balancer(const struct nbrec_logical_switch *row, struct nbrec_load_balancer **load_balancer, size_t n_load_balancer)
{
    ...
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_UUID, OVSDB_TYPE_VOID);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_switch_columns[ NBREC_LOGICAL_SWITCH_COL_LOAD_BALANCER ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_SWITCH]);
}
...
nbrec_logical_switch_index_set_name(const struct nbrec_logical_switch *row, const char *name)
{
    ...
    key->string = CONST_CAST(char *, name);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_switch_columns[ NBREC_LOGICAL_SWITCH_COL_NAME ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_SWITCH]);
}
...
nbrec_logical_switch_index_set_ports(const struct nbrec_logical_switch *row, struct nbrec_logical_switch_port **ports, size_t n_ports)
{
    ...
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_UUID, OVSDB_TYPE_VOID);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_switch_columns[ NBREC_LOGICAL_SWITCH_COL_PORTS ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_SWITCH]);
}
...
nbrec_logical_switch_index_set_qos_rules(const struct nbrec_logical_switch *row, struct nbrec_qos **qos_rules, size_t n_qos_rules)
{
    ...
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_UUID, OVSDB_TYPE_VOID);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_switch_columns[ NBREC_LOGICAL_SWITCH_COL_QOS_RULES ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_SWITCH]);
}
...
struct ovsdb_idl_column nbrec_logical_switch_columns[NBREC_LOGICAL_SWITCH_N_COLUMNS] = {
    [NBREC_LOGICAL_SWITCH_COL_ACLS] = {
         ...
         .type = {
            .key = {
               .type = OVSDB_TYPE_UUID,
               .uuid = { .refTableName = "ACL", .refType = OVSDB_REF_STRONG },
            },
            .value = OVSDB_BASE_VOID_INIT,
            .n_min = 0,
            .n_max = UINT_MAX,
    ...
    [NBREC_LOGICAL_SWITCH_COL_DNS_RECORDS] = {
         ...
         .type = {
            .key = {
               .type = OVSDB_TYPE_UUID,
               .uuid = { .refTableName = "DNS", .refType = OVSDB_REF_WEAK },
            },
            .value = OVSDB_BASE_VOID_INIT,
            .n_min = 0,
            .n_max = UINT_MAX,
    ...
    [NBREC_LOGICAL_SWITCH_COL_PORTS] = {
         ...
         .type = {
            .key = {
               .type = OVSDB_TYPE_UUID,
               .uuid = { .refTableName = "Logical_Switch_Port", .refType = OVSDB_REF_STRONG },
            },
            .value = OVSDB_BASE_VOID_INIT,
            .n_min = 0,
            .n_max = UINT_MAX,
...
const struct nbrec_logical_switch_port_table *
nbrec_logical_switch_port_table_get(const struct ovsdb_idl *idl)
{
    return (const struct nbrec_logical_switch_port_table *) idl;
}

const struct nbrec_logical_switch_port *
nbrec_logical_switch_port_table_first(const struct nbrec_logical_switch_port_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_logical_switch_port_first(idl);
}


const struct nbrec_logical_switch_port *
nbrec_logical_switch_port_table_track_get_first(const struct nbrec_logical_switch_port_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return nbrec_logical_switch_port_track_get_first(idl);
}
...
/* Searches table "Logical_Switch_Port" for a row with UUID 'uuid'.  Returns
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct nbrec_logical_switch_port *
nbrec_logical_switch_port_table_get_for_uuid(const struct nbrec_logical_switch_port_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_logical_switch_port_get_for_uuid(idl, uuid);
}
...
struct nbrec_logical_switch_port *
nbrec_logical_switch_port_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_switch_port);
    return ALIGNED_CAST(struct nbrec_logical_switch_port *, ovsdb_idl_index_init_row(index));
}

struct nbrec_logical_switch_port *
nbrec_logical_switch_port_index_find(struct ovsdb_idl_index *index, const struct nbrec_logical_switch_port *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_switch_port);
    return nbrec_logical_switch_port_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
nbrec_logical_switch_port_index_compare(
    struct ovsdb_idl_index *index, 
    const struct nbrec_logical_switch_port *a, 
    const struct nbrec_logical_switch_port *b)
{
    return ovsdb_idl_index_compare(index, &a->header_, &b->header_);
}

struct ovsdb_idl_cursor
nbrec_logical_switch_port_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_switch_port);
    return ovsdb_idl_cursor_first(index);
}
...
struct ovsdb_idl_cursor
nbrec_logical_switch_port_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct nbrec_logical_switch_port *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_switch_port);
    return ovsdb_idl_cursor_first_eq(index, &target->header_);
}
...
struct ovsdb_idl_cursor
nbrec_logical_switch_port_cursor_first_ge(
    struct ovsdb_idl_index *index, const struct nbrec_logical_switch_port *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_logical_switch_port);
    return ovsdb_idl_cursor_first_ge(index, &target->header_);
...
struct nbrec_logical_switch_port *
nbrec_logical_switch_port_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return nbrec_logical_switch_port_cast(ovsdb_idl_cursor_data(cursor));
}
...
nbrec_logical_switch_port_index_set_addresses(const struct nbrec_logical_switch_port *row, const char **addresses, size_t n_addresses)
{
    ...
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_STRING, OVSDB_TYPE_VOID);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_switch_port_columns[ NBREC_LOGICAL_SWITCH_PORT_COL_ADDRESSES ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_SWITCH_PORT]);
}
...
nbrec_logical_switch_port_index_set_dhcpv4_options(const struct nbrec_logical_switch_port *row, const struct nbrec_dhcp_options *dhcpv4_options)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_switch_port_columns[ NBREC_LOGICAL_SWITCH_PORT_COL_DHCPV4_OPTIONS ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_SWITCH_PORT]);
}
...
nbrec_logical_switch_port_index_set_dhcpv6_options(const struct nbrec_logical_switch_port *row, const struct nbrec_dhcp_options *dhcpv6_options)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_switch_port_columns[ NBREC_LOGICAL_SWITCH_PORT_COL_DHCPV6_OPTIONS ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_SWITCH_PORT]);
}
...
nbrec_logical_switch_port_index_set_dynamic_addresses(const struct nbrec_logical_switch_port *row, const char *dynamic_addresses)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_switch_port_columns[ NBREC_LOGICAL_SWITCH_PORT_COL_DYNAMIC_ADDRESSES ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_SWITCH_PORT]);
}
...
nbrec_logical_switch_port_index_set_enabled(const struct nbrec_logical_switch_port *row, const bool *enabled, size_t n_enabled)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_switch_port_columns[ NBREC_LOGICAL_SWITCH_PORT_COL_ENABLED ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_SWITCH_PORT]);
}
...
nbrec_logical_switch_port_index_set_name(const struct nbrec_logical_switch_port *row, const char *name)
{
    ...
    key->string = CONST_CAST(char *, name);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_switch_port_columns[ NBREC_LOGICAL_SWITCH_PORT_COL_NAME ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_SWITCH_PORT]);
}
...
nbrec_logical_switch_port_index_set_parent_name(const struct nbrec_logical_switch_port *row, const char *parent_name)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_switch_port_columns[ NBREC_LOGICAL_SWITCH_PORT_COL_PARENT_NAME ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_SWITCH_PORT]);
}
...
nbrec_logical_switch_port_index_set_port_security(const struct nbrec_logical_switch_port *row, const char **port_security, size_t n_port_security)
{
    ...
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_STRING, OVSDB_TYPE_VOID);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_switch_port_columns[ NBREC_LOGICAL_SWITCH_PORT_COL_PORT_SECURITY ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_SWITCH_PORT]);
}
...
nbrec_logical_switch_port_index_set_tag(const struct nbrec_logical_switch_port *row, const int64_t *tag, size_t n_tag)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_switch_port_columns[ NBREC_LOGICAL_SWITCH_PORT_COL_TAG ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_SWITCH_PORT]);
}
...
nbrec_logical_switch_port_index_set_tag_request(const struct nbrec_logical_switch_port *row, const int64_t *tag_request, size_t n_tag_request)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_switch_port_columns[ NBREC_LOGICAL_SWITCH_PORT_COL_TAG_REQUEST ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_SWITCH_PORT]);
}
...
nbrec_logical_switch_port_index_set_type(const struct nbrec_logical_switch_port *row, const char *type)
{
    ...
    key->string = CONST_CAST(char *, type);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_switch_port_columns[ NBREC_LOGICAL_SWITCH_PORT_COL_TYPE ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_SWITCH_PORT]);
}
...
nbrec_logical_switch_port_index_set_up(const struct nbrec_logical_switch_port *row, const bool *up, size_t n_up)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_logical_switch_port_columns[ NBREC_LOGICAL_SWITCH_PORT_COL_UP ], &datum, &nbrec_table_classes[NBREC_TABLE_LOGICAL_SWITCH_PORT]);
}
...
struct ovsdb_idl_column nbrec_logical_switch_port_columns[NBREC_LOGICAL_SWITCH_PORT_N_COLUMNS] = {
    ...
    [NBREC_LOGICAL_SWITCH_PORT_COL_DHCPV4_OPTIONS] = {
         ...
         .type = {
            .key = {
               .type = OVSDB_TYPE_UUID,
               .uuid = { .refTableName = "DHCP_Options", .refType = OVSDB_REF_WEAK },
            },
            .value = OVSDB_BASE_VOID_INIT,
            .n_min = 0,
            .n_max = 1,
...
nbrec_meter_table_get(const struct ovsdb_idl *idl)
{
    return (const struct nbrec_meter_table *) idl;
}

const struct nbrec_meter *
nbrec_meter_table_first(const struct nbrec_meter_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_meter_first(idl);
}


const struct nbrec_meter *
nbrec_meter_table_track_get_first(const struct nbrec_meter_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return nbrec_meter_track_get_first(idl);
}


static void
nbrec_meter_parse_bands(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct nbrec_meter *row = nbrec_meter_cast(row_);
    row->bands = NULL;
    row->n_bands = 0;
    for (size_t i = 0; i < datum->n; i++) {
        struct nbrec_meter_band *keyRow = nbrec_meter_band_cast(ovsdb_idl_get_row_arc(row_, &nbrec_table_meter_band, &datum->keys[i].uuid));
        if (!keyRow) {
            continue;
        }
        if (!row->n_bands) {
...
static void
nbrec_meter_parse_external_ids(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct nbrec_meter *row = nbrec_meter_cast(row_);
    smap_init(&row->external_ids);
    for (size_t i = 0; i < datum->n; i++) {
...
static void
nbrec_meter_parse_name(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct nbrec_meter *row = nbrec_meter_cast(row_);
...
static void
nbrec_meter_parse_unit(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct nbrec_meter *row = nbrec_meter_cast(row_);
...
static void
nbrec_meter_unparse_bands(struct ovsdb_idl_row *row_)
{
    struct nbrec_meter *row = nbrec_meter_cast(row_);
    free(row->bands);
}
...
static void
nbrec_meter_unparse_external_ids(struct ovsdb_idl_row *row_)
{
    struct nbrec_meter *row = nbrec_meter_cast(row_);
    smap_destroy(&row->external_ids);
}
...
static void
nbrec_meter_unparse_name(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}

static void
nbrec_meter_unparse_unit(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}

static void
nbrec_meter_init__(struct ovsdb_idl_row *row)
{
    nbrec_meter_init(nbrec_meter_cast(row));
}

/* Clears the contents of 'row' in table "Meter". */
void
nbrec_meter_init(struct nbrec_meter *row)
{
    memset(row, 0, sizeof *row); 
    smap_init(&row->external_ids);
    row->name = "";
 ...
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct nbrec_meter *
nbrec_meter_get_for_uuid(const struct ovsdb_idl *idl, const struct uuid *uuid)
{
    return nbrec_meter_cast(ovsdb_idl_get_row_for_uuid(idl, &nbrec_table_meter, uuid));
}

/* Searches table "Meter" for a row with UUID 'uuid'.  Returns
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct nbrec_meter *
nbrec_meter_table_get_for_uuid(const struct nbrec_meter_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_meter_get_for_uuid(idl, uuid);
}
 ...
 * visited twice or not at apply. */
const struct nbrec_meter *
nbrec_meter_first(const struct ovsdb_idl *idl)
{
    return nbrec_meter_cast(ovsdb_idl_first_row(idl, &nbrec_table_meter));
}

/* Returns a row following 'row' within its table, or a null pointer if 'row'
 * is the last row in its table. */
const struct nbrec_meter *
nbrec_meter_next(const struct nbrec_meter *row)
{
    return nbrec_meter_cast(ovsdb_idl_next_row(&row->header_));
}

unsigned int nbrec_meter_get_seqno(const struct ovsdb_idl *idl)
{
    return ovsdb_idl_table_get_seqno(idl, &nbrec_table_meter);
}

unsigned int nbrec_meter_row_get_seqno(const struct nbrec_meter *row, enum ovsdb_idl_change change)
{
    return ovsdb_idl_row_get_seqno(&row->header_, change);
}

const struct nbrec_meter *
nbrec_meter_track_get_first(const struct ovsdb_idl *idl)
{
    return nbrec_meter_cast(ovsdb_idl_track_get_first(idl, &nbrec_table_meter));
}

const struct nbrec_meter
*nbrec_meter_track_get_next(const struct nbrec_meter *row)
{
    return nbrec_meter_cast(ovsdb_idl_track_get_next(&row->header_));
}
 ...
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
nbrec_meter_delete(const struct nbrec_meter *row)
{
    ovsdb_idl_txn_delete(&row->header_);
}
 ...
 * data to be to be committed by the UUID assigned by ovsdb-server. */
struct nbrec_meter *
nbrec_meter_insert(struct ovsdb_idl_txn *txn)
{
    return nbrec_meter_cast(ovsdb_idl_txn_insert(txn, &nbrec_table_meter, NULL));
}

bool
nbrec_meter_is_updated(const struct nbrec_meter *row, enum nbrec_meter_column_id column)
{
    return ovsdb_idl_track_is_updated(&row->header_, &nbrec_meter_columns[column]);
}
 ...
 *     nbrec_meter_set_bands()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * nbrec_meter_set_bands() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
nbrec_meter_verify_bands(const struct nbrec_meter *row)
{
    ovsdb_idl_txn_verify(&row->header_, &nbrec_meter_col_bands);
}
 ...
 *
 *   - If "external_ids" has already been modified (with
 *     nbrec_meter_set_external_ids()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * nbrec_meter_set_external_ids() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
nbrec_meter_verify_external_ids(const struct nbrec_meter *row)
{
    ovsdb_idl_txn_verify(&row->header_, &nbrec_meter_col_external_ids);
}
 ...
 *
 *   - If "name" has already been modified (with
 *     nbrec_meter_set_name()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * nbrec_meter_set_name() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
nbrec_meter_verify_name(const struct nbrec_meter *row)
{
    ovsdb_idl_txn_verify(&row->header_, &nbrec_meter_col_name);
}
 ...
 *     nbrec_meter_set_unit()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * nbrec_meter_set_unit() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
nbrec_meter_verify_unit(const struct nbrec_meter *row)
{
    ovsdb_idl_txn_verify(&row->header_, &nbrec_meter_col_unit);
}
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "bands" member in nbrec_meter. */
const struct ovsdb_datum *
nbrec_meter_get_bands(const struct nbrec_meter *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_UUID);
    return ovsdb_idl_read(&row->header_, &nbrec_meter_col_bands);
}

/* Returns the "external_ids" column's value from the "Meter" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "external_ids" member in nbrec_meter. */
const struct ovsdb_datum *
nbrec_meter_get_external_ids(const struct nbrec_meter *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED,
	enum ovsdb_atomic_type value_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    ovs_assert(value_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &nbrec_meter_col_external_ids);
}
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "name" member in nbrec_meter. */
const struct ovsdb_datum *
nbrec_meter_get_name(const struct nbrec_meter *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &nbrec_meter_col_name);
}
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "unit" member in nbrec_meter. */
const struct ovsdb_datum *
nbrec_meter_get_unit(const struct nbrec_meter *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &nbrec_meter_col_unit);
}
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_meter_set_bands(const struct nbrec_meter *row, struct nbrec_meter_band **bands, size_t n_bands)
{
    struct ovsdb_datum datum;
    ...
    ovsdb_idl_txn_write(&row->header_, &nbrec_meter_col_bands, &datum);
}

/* Sets the "external_ids" column's value from the "Meter" table in 'row'
 * to 'external_ids'.
 *
 * The caller retains ownership of 'external_ids' and everything in it. */
void
nbrec_meter_set_external_ids(const struct nbrec_meter *row, const struct smap *external_ids)
{
    struct ovsdb_datum datum;
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_meter_set_name(const struct nbrec_meter *row, const char *name)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    key.string = CONST_CAST(char *, name);
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &nbrec_meter_col_name, &datum);
}
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_meter_set_unit(const struct nbrec_meter *row, const char *unit)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &nbrec_meter_col_unit, &datum);
}
 ...
 */
void
nbrec_meter_update_bands_addvalue(const struct nbrec_meter *row, const struct nbrec_meter_band *new_value)
{
    struct ovsdb_datum *datum;
 ...
 */
void
nbrec_meter_update_bands_delvalue(const struct nbrec_meter *row, const struct nbrec_meter_band *delete_value)
{
    struct ovsdb_datum *datum;
...
/* Sets an element of the "external_ids" map column from the "Meter" table in 'row'
 * to 'new_value' given the key value 'new_key'.
 *
 */
void
nbrec_meter_update_external_ids_setkey(const struct nbrec_meter *row, const char *new_key, const char *new_value)
{
    struct ovsdb_datum *datum;
...
/* Deletes an element of the "external_ids" map column from the "Meter" table in 'row'
 * given the key value 'delete_key'.
 *
 */
void
nbrec_meter_update_external_ids_delkey(const struct nbrec_meter *row, const char *delete_key)
{
    struct ovsdb_datum *datum;
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_meter_add_clause_bands(struct ovsdb_idl_condition *cond, enum ovsdb_function function, struct uuid **bands, size_t n_bands)
{
    struct ovsdb_datum datum;
    datum.n = n_bands;
    datum.keys = n_bands ? xmalloc(n_bands * sizeof *datum.keys) : NULL;
 ...
 * The caller retains ownership of 'external_ids' and everything in it. */
void
nbrec_meter_add_clause_external_ids(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const struct smap *external_ids)
{
    struct ovsdb_datum datum;
    ...
    ovsdb_datum_destroy(&datum, &nbrec_meter_col_external_ids.type);
}
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_meter_add_clause_name(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const char *name)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_meter_add_clause_unit(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const char *unit)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
 ...
 */
void
nbrec_meter_index_destroy_row(const struct nbrec_meter *row)
{
    ovsdb_idl_index_destroy_row(&row->header_);
}
...
struct nbrec_meter *
nbrec_meter_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_meter);
    return ALIGNED_CAST(struct nbrec_meter *, ovsdb_idl_index_init_row(index));
}

struct nbrec_meter *
nbrec_meter_index_find(struct ovsdb_idl_index *index, const struct nbrec_meter *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_meter);
    return nbrec_meter_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
nbrec_meter_index_compare(
    struct ovsdb_idl_index *index, 
    const struct nbrec_meter *a, 
    const struct nbrec_meter *b)
{
    return ovsdb_idl_index_compare(index, &a->header_, &b->header_);
}

struct ovsdb_idl_cursor
nbrec_meter_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_meter);
    return ovsdb_idl_cursor_first(index);
}
...
struct ovsdb_idl_cursor
nbrec_meter_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct nbrec_meter *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_meter);
    return ovsdb_idl_cursor_first_eq(index, &target->header_);
}
...
struct nbrec_meter *
nbrec_meter_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return nbrec_meter_cast(ovsdb_idl_cursor_data(cursor));
}
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_meter_index_set_bands(const struct nbrec_meter *row, struct nbrec_meter_band **bands, size_t n_bands)
{
    struct ovsdb_datum datum;
    size_t i;
    ...
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_UUID, OVSDB_TYPE_VOID);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_meter_columns[ NBREC_METER_COL_BANDS ], &datum, &nbrec_table_classes[NBREC_TABLE_METER]);
}
 ...
 * The caller retains ownership of 'external_ids' and everything in it. */
void
nbrec_meter_index_set_external_ids(const struct nbrec_meter *row, const struct smap *external_ids)
{
    struct ovsdb_datum *datum = xmalloc(sizeof(struct ovsdb_datum));
    ...
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_),
                          &nbrec_meter_columns[NBREC_METER_COL_EXTERNAL_IDS],
                          datum,
                          &nbrec_table_classes[NBREC_TABLE_METER]);
}
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_meter_index_set_name(const struct nbrec_meter *row, const char *name)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof(union ovsdb_atom));
    ...
    key->string = CONST_CAST(char *, name);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_meter_columns[ NBREC_METER_COL_NAME ], &datum, &nbrec_table_classes[NBREC_TABLE_METER]);
}
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_meter_index_set_unit(const struct nbrec_meter *row, const char *unit)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof(union ovsdb_atom));
    ...
    key->string = CONST_CAST(char *, unit);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_meter_columns[ NBREC_METER_COL_UNIT ], &datum, &nbrec_table_classes[NBREC_TABLE_METER]);
}

struct ovsdb_idl_column nbrec_meter_columns[NBREC_METER_N_COLUMNS];

void
nbrec_meter_set_condition(struct ovsdb_idl *idl, struct ovsdb_idl_condition *condition)
{
    ovsdb_idl_set_condition(idl, &nbrec_table_meter, condition);
}
...
struct ovsdb_idl_column nbrec_meter_columns[NBREC_METER_N_COLUMNS] = {
    [NBREC_METER_COL_BANDS] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = nbrec_meter_parse_bands,
         .unparse = nbrec_meter_unparse_bands,
    },
    ...
    [NBREC_METER_COL_EXTERNAL_IDS] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = nbrec_meter_parse_external_ids,
         .unparse = nbrec_meter_unparse_external_ids,
    },
    ...
    [NBREC_METER_COL_NAME] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = nbrec_meter_parse_name,
         .unparse = nbrec_meter_unparse_name,
    },
    ...
    [NBREC_METER_COL_UNIT] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = nbrec_meter_parse_unit,
         .unparse = nbrec_meter_unparse_unit,
    },
...
const struct nbrec_meter_band_table *
nbrec_meter_band_table_get(const struct ovsdb_idl *idl)
{
    return (const struct nbrec_meter_band_table *) idl;
}

const struct nbrec_meter_band *
nbrec_meter_band_table_first(const struct nbrec_meter_band_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_meter_band_first(idl);
}


const struct nbrec_meter_band *
nbrec_meter_band_table_track_get_first(const struct nbrec_meter_band_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return nbrec_meter_band_track_get_first(idl);
}


static void
nbrec_meter_band_parse_action(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct nbrec_meter_band *row = nbrec_meter_band_cast(row_);
...
static void
nbrec_meter_band_parse_burst_size(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct nbrec_meter_band *row = nbrec_meter_band_cast(row_);
...
static void
nbrec_meter_band_parse_external_ids(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct nbrec_meter_band *row = nbrec_meter_band_cast(row_);
    smap_init(&row->external_ids);
    for (size_t i = 0; i < datum->n; i++) {
...
static void
nbrec_meter_band_parse_rate(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct nbrec_meter_band *row = nbrec_meter_band_cast(row_);
...
static void
nbrec_meter_band_unparse_action(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}

static void
nbrec_meter_band_unparse_burst_size(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}

static void
nbrec_meter_band_unparse_external_ids(struct ovsdb_idl_row *row_)
{
    struct nbrec_meter_band *row = nbrec_meter_band_cast(row_);
    smap_destroy(&row->external_ids);
}
...
static void
nbrec_meter_band_unparse_rate(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}

static void
nbrec_meter_band_init__(struct ovsdb_idl_row *row)
{
    nbrec_meter_band_init(nbrec_meter_band_cast(row));
}

/* Clears the contents of 'row' in table "Meter_Band". */
void
nbrec_meter_band_init(struct nbrec_meter_band *row)
{
    memset(row, 0, sizeof *row); 
    row->action = "";
    smap_init(&row->external_ids);
 ...
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct nbrec_meter_band *
nbrec_meter_band_get_for_uuid(const struct ovsdb_idl *idl, const struct uuid *uuid)
{
    return nbrec_meter_band_cast(ovsdb_idl_get_row_for_uuid(idl, &nbrec_table_meter_band, uuid));
}

/* Searches table "Meter_Band" for a row with UUID 'uuid'.  Returns
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct nbrec_meter_band *
nbrec_meter_band_table_get_for_uuid(const struct nbrec_meter_band_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_meter_band_get_for_uuid(idl, uuid);
}
 ...
 * visited twice or not at apply. */
const struct nbrec_meter_band *
nbrec_meter_band_first(const struct ovsdb_idl *idl)
{
    return nbrec_meter_band_cast(ovsdb_idl_first_row(idl, &nbrec_table_meter_band));
}

/* Returns a row following 'row' within its table, or a null pointer if 'row'
 * is the last row in its table. */
const struct nbrec_meter_band *
nbrec_meter_band_next(const struct nbrec_meter_band *row)
{
    return nbrec_meter_band_cast(ovsdb_idl_next_row(&row->header_));
}

unsigned int nbrec_meter_band_get_seqno(const struct ovsdb_idl *idl)
{
    return ovsdb_idl_table_get_seqno(idl, &nbrec_table_meter_band);
}

unsigned int nbrec_meter_band_row_get_seqno(const struct nbrec_meter_band *row, enum ovsdb_idl_change change)
{
    return ovsdb_idl_row_get_seqno(&row->header_, change);
}

const struct nbrec_meter_band *
nbrec_meter_band_track_get_first(const struct ovsdb_idl *idl)
{
    return nbrec_meter_band_cast(ovsdb_idl_track_get_first(idl, &nbrec_table_meter_band));
}

const struct nbrec_meter_band
*nbrec_meter_band_track_get_next(const struct nbrec_meter_band *row)
{
    return nbrec_meter_band_cast(ovsdb_idl_track_get_next(&row->header_));
}
 ...
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
nbrec_meter_band_delete(const struct nbrec_meter_band *row)
{
    ovsdb_idl_txn_delete(&row->header_);
}
 ...
 * but the IDL will replace any uses of the provisional UUID in the
 * data to be to be committed by the UUID assigned by ovsdb-server. */
struct nbrec_meter_band *
nbrec_meter_band_insert(struct ovsdb_idl_txn *txn)
{
    return nbrec_meter_band_cast(ovsdb_idl_txn_insert(txn, &nbrec_table_meter_band, NULL));
}

bool
nbrec_meter_band_is_updated(const struct nbrec_meter_band *row, enum nbrec_meter_band_column_id column)
{
    return ovsdb_idl_track_is_updated(&row->header_, &nbrec_meter_band_columns[column]);
}
 ...
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     nbrec_meter_band_insert()).
 *
 *   - If "action" has already been modified (with
 *     nbrec_meter_band_set_action()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * nbrec_meter_band_set_action() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
nbrec_meter_band_verify_action(const struct nbrec_meter_band *row)
{
    ovsdb_idl_txn_verify(&row->header_, &nbrec_meter_band_col_action);
}
 ...
 *     nbrec_meter_band_set_burst_size()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * nbrec_meter_band_set_burst_size() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
nbrec_meter_band_verify_burst_size(const struct nbrec_meter_band *row)
{
    ovsdb_idl_txn_verify(&row->header_, &nbrec_meter_band_col_burst_size);
}
 ...
 *
 *   - If "external_ids" has already been modified (with
 *     nbrec_meter_band_set_external_ids()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * nbrec_meter_band_set_external_ids() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
nbrec_meter_band_verify_external_ids(const struct nbrec_meter_band *row)
{
    ovsdb_idl_txn_verify(&row->header_, &nbrec_meter_band_col_external_ids);
}
 ...
 *     nbrec_meter_band_set_rate()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * nbrec_meter_band_set_rate() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
nbrec_meter_band_verify_rate(const struct nbrec_meter_band *row)
{
    ovsdb_idl_txn_verify(&row->header_, &nbrec_meter_band_col_rate);
}
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "action" member in nbrec_meter_band. */
const struct ovsdb_datum *
nbrec_meter_band_get_action(const struct nbrec_meter_band *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &nbrec_meter_band_col_action);
}
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "burst_size" member in nbrec_meter_band. */
const struct ovsdb_datum *
nbrec_meter_band_get_burst_size(const struct nbrec_meter_band *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_INTEGER);
    return ovsdb_idl_read(&row->header_, &nbrec_meter_band_col_burst_size);
}

/* Returns the "external_ids" column's value from the "Meter_Band" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "external_ids" member in nbrec_meter_band. */
const struct ovsdb_datum *
nbrec_meter_band_get_external_ids(const struct nbrec_meter_band *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED,
	enum ovsdb_atomic_type value_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    ovs_assert(value_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &nbrec_meter_band_col_external_ids);
}
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "rate" member in nbrec_meter_band. */
const struct ovsdb_datum *
nbrec_meter_band_get_rate(const struct nbrec_meter_band *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_INTEGER);
    return ovsdb_idl_read(&row->header_, &nbrec_meter_band_col_rate);
}
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_meter_band_set_action(const struct nbrec_meter_band *row, const char *action)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    key.string = CONST_CAST(char *, action);
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &nbrec_meter_band_col_action, &datum);
}
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_meter_band_set_burst_size(const struct nbrec_meter_band *row, int64_t burst_size)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &nbrec_meter_band_col_burst_size, &datum);
}

/* Sets the "external_ids" column's value from the "Meter_Band" table in 'row'
 * to 'external_ids'.
 *
 * The caller retains ownership of 'external_ids' and everything in it. */
void
nbrec_meter_band_set_external_ids(const struct nbrec_meter_band *row, const struct smap *external_ids)
{
    struct ovsdb_datum datum;
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_meter_band_set_rate(const struct nbrec_meter_band *row, int64_t rate)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    key.integer = rate;
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &nbrec_meter_band_col_rate, &datum);
}

/* Sets an element of the "external_ids" map column from the "Meter_Band" table in 'row'
 * to 'new_value' given the key value 'new_key'.
 *
 */
void
nbrec_meter_band_update_external_ids_setkey(const struct nbrec_meter_band *row, const char *new_key, const char *new_value)
{
    struct ovsdb_datum *datum;
...
/* Deletes an element of the "external_ids" map column from the "Meter_Band" table in 'row'
 * given the key value 'delete_key'.
 *
 */
void
nbrec_meter_band_update_external_ids_delkey(const struct nbrec_meter_band *row, const char *delete_key)
{
    struct ovsdb_datum *datum;
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_meter_band_add_clause_action(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const char *action)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_meter_band_add_clause_burst_size(struct ovsdb_idl_condition *cond, enum ovsdb_function function, int64_t burst_size)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    ovsdb_idl_condition_add_clause(cond,
                          function,
                          &nbrec_meter_band_col_burst_size,
                          &datum);
}
 ...
 * The caller retains ownership of 'external_ids' and everything in it. */
void
nbrec_meter_band_add_clause_external_ids(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const struct smap *external_ids)
{
    struct ovsdb_datum datum;
    ...
    ovsdb_datum_destroy(&datum, &nbrec_meter_band_col_external_ids.type);
}
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_meter_band_add_clause_rate(struct ovsdb_idl_condition *cond, enum ovsdb_function function, int64_t rate)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
 ...
 */
void
nbrec_meter_band_index_destroy_row(const struct nbrec_meter_band *row)
{
    ovsdb_idl_index_destroy_row(&row->header_);
}
...
struct nbrec_meter_band *
nbrec_meter_band_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_meter_band);
    return ALIGNED_CAST(struct nbrec_meter_band *, ovsdb_idl_index_init_row(index));
}

struct nbrec_meter_band *
nbrec_meter_band_index_find(struct ovsdb_idl_index *index, const struct nbrec_meter_band *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_meter_band);
    return nbrec_meter_band_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
nbrec_meter_band_index_compare(
    struct ovsdb_idl_index *index, 
    const struct nbrec_meter_band *a, 
    const struct nbrec_meter_band *b)
{
    return ovsdb_idl_index_compare(index, &a->header_, &b->header_);
}

struct ovsdb_idl_cursor
nbrec_meter_band_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_meter_band);
    return ovsdb_idl_cursor_first(index);
}
...
struct ovsdb_idl_cursor
nbrec_meter_band_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct nbrec_meter_band *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_meter_band);
    return ovsdb_idl_cursor_first_eq(index, &target->header_);
}
...
struct ovsdb_idl_cursor
nbrec_meter_band_cursor_first_ge(
    struct ovsdb_idl_index *index, const struct nbrec_meter_band *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_meter_band);
    return ovsdb_idl_cursor_first_ge(index, &target->header_);
...
struct nbrec_meter_band *
nbrec_meter_band_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return nbrec_meter_band_cast(ovsdb_idl_cursor_data(cursor));
}
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_meter_band_index_set_action(const struct nbrec_meter_band *row, const char *action)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof(union ovsdb_atom));
    ...
    key->string = CONST_CAST(char *, action);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_meter_band_columns[ NBREC_METER_BAND_COL_ACTION ], &datum, &nbrec_table_classes[NBREC_TABLE_METER_BAND]);
}
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_meter_band_index_set_burst_size(const struct nbrec_meter_band *row, int64_t burst_size)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof(union ovsdb_atom));
    ...
    key->integer = burst_size;
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_meter_band_columns[ NBREC_METER_BAND_COL_BURST_SIZE ], &datum, &nbrec_table_classes[NBREC_TABLE_METER_BAND]);
}
 ...
 * The caller retains ownership of 'external_ids' and everything in it. */
void
nbrec_meter_band_index_set_external_ids(const struct nbrec_meter_band *row, const struct smap *external_ids)
{
    struct ovsdb_datum *datum = xmalloc(sizeof(struct ovsdb_datum));
    ...
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_),
                          &nbrec_meter_band_columns[NBREC_METER_BAND_COL_EXTERNAL_IDS],
                          datum,
                          &nbrec_table_classes[NBREC_TABLE_METER_BAND]);
}
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_meter_band_index_set_rate(const struct nbrec_meter_band *row, int64_t rate)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof(union ovsdb_atom));
    ...
    key->integer = rate;
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_meter_band_columns[ NBREC_METER_BAND_COL_RATE ], &datum, &nbrec_table_classes[NBREC_TABLE_METER_BAND]);
}

struct ovsdb_idl_column nbrec_meter_band_columns[NBREC_METER_BAND_N_COLUMNS];

void
nbrec_meter_band_set_condition(struct ovsdb_idl *idl, struct ovsdb_idl_condition *condition)
{
    ovsdb_idl_set_condition(idl, &nbrec_table_meter_band, condition);
}
...
struct ovsdb_idl_column nbrec_meter_band_columns[NBREC_METER_BAND_N_COLUMNS] = {
    [NBREC_METER_BAND_COL_ACTION] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = nbrec_meter_band_parse_action,
         .unparse = nbrec_meter_band_unparse_action,
    },
    ...
    [NBREC_METER_BAND_COL_BURST_SIZE] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = nbrec_meter_band_parse_burst_size,
         .unparse = nbrec_meter_band_unparse_burst_size,
    },
    ...
    [NBREC_METER_BAND_COL_EXTERNAL_IDS] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = nbrec_meter_band_parse_external_ids,
         .unparse = nbrec_meter_band_unparse_external_ids,
    },
    ...
    [NBREC_METER_BAND_COL_RATE] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = nbrec_meter_band_parse_rate,
         .unparse = nbrec_meter_band_unparse_rate,
    },
...
const struct nbrec_nat_table *
nbrec_nat_table_get(const struct ovsdb_idl *idl)
{
    return (const struct nbrec_nat_table *) idl;
}

const struct nbrec_nat *
nbrec_nat_table_first(const struct nbrec_nat_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_nat_first(idl);
}


const struct nbrec_nat *
nbrec_nat_table_track_get_first(const struct nbrec_nat_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return nbrec_nat_track_get_first(idl);
}


static void
nbrec_nat_parse_external_ids(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
...
static void
nbrec_nat_unparse_external_ids(struct ovsdb_idl_row *row_)
{
    struct nbrec_nat *row = nbrec_nat_cast(row_);
    smap_destroy(&row->external_ids);
}
...
/* Searches table "NAT" for a row with UUID 'uuid'.  Returns
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct nbrec_nat *
nbrec_nat_table_get_for_uuid(const struct nbrec_nat_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_nat_get_for_uuid(idl, uuid);
}
 ...
 *
 *   - If "external_ids" has already been modified (with
 *     nbrec_nat_set_external_ids()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * nbrec_nat_set_external_ids() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
nbrec_nat_verify_external_ids(const struct nbrec_nat *row)
{
    ovsdb_idl_txn_verify(&row->header_, &nbrec_nat_col_external_ids);
}
...
/* Returns the "external_ids" column's value from the "NAT" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "external_ids" member in nbrec_nat. */
const struct ovsdb_datum *
nbrec_nat_get_external_ids(const struct nbrec_nat *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED,
	enum ovsdb_atomic_type value_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    ovs_assert(value_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &nbrec_nat_col_external_ids);
}
...
/* Sets the "external_ids" column's value from the "NAT" table in 'row'
 * to 'external_ids'.
 *
 * The caller retains ownership of 'external_ids' and everything in it. */
void
nbrec_nat_set_external_ids(const struct nbrec_nat *row, const struct smap *external_ids)
{
    struct ovsdb_datum datum;
...
/* Sets an element of the "external_ids" map column from the "NAT" table in 'row'
 * to 'new_value' given the key value 'new_key'.
 *
 */
void
nbrec_nat_update_external_ids_setkey(const struct nbrec_nat *row, const char *new_key, const char *new_value)
{
    struct ovsdb_datum *datum;
...
/* Deletes an element of the "external_ids" map column from the "NAT" table in 'row'
 * given the key value 'delete_key'.
 *
 */
void
nbrec_nat_update_external_ids_delkey(const struct nbrec_nat *row, const char *delete_key)
{
    struct ovsdb_datum *datum;
 ...
 * The caller retains ownership of 'external_ids' and everything in it. */
void
nbrec_nat_add_clause_external_ids(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const struct smap *external_ids)
{
    struct ovsdb_datum datum;
    ...
    ovsdb_datum_destroy(&datum, &nbrec_nat_col_external_ids.type);
}
...
struct nbrec_nat *
nbrec_nat_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_nat);
    return ALIGNED_CAST(struct nbrec_nat *, ovsdb_idl_index_init_row(index));
}

struct nbrec_nat *
nbrec_nat_index_find(struct ovsdb_idl_index *index, const struct nbrec_nat *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_nat);
    return nbrec_nat_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
nbrec_nat_index_compare(
    struct ovsdb_idl_index *index, 
    const struct nbrec_nat *a, 
    const struct nbrec_nat *b)
{
    return ovsdb_idl_index_compare(index, &a->header_, &b->header_);
}

struct ovsdb_idl_cursor
nbrec_nat_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_nat);
    return ovsdb_idl_cursor_first(index);
}
...
struct ovsdb_idl_cursor
nbrec_nat_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct nbrec_nat *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_nat);
    return ovsdb_idl_cursor_first_eq(index, &target->header_);
}
...
struct nbrec_nat *
nbrec_nat_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return nbrec_nat_cast(ovsdb_idl_cursor_data(cursor));
}
 ...
 * The caller retains ownership of 'external_ids' and everything in it. */
void
nbrec_nat_index_set_external_ids(const struct nbrec_nat *row, const struct smap *external_ids)
{
    struct ovsdb_datum *datum = xmalloc(sizeof(struct ovsdb_datum));
    ...
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_),
                          &nbrec_nat_columns[NBREC_NAT_COL_EXTERNAL_IDS],
                          datum,
                          &nbrec_table_classes[NBREC_TABLE_NAT]);
}
...
nbrec_nat_index_set_external_ip(const struct nbrec_nat *row, const char *external_ip)
{
    ...
    key->string = CONST_CAST(char *, external_ip);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_nat_columns[ NBREC_NAT_COL_EXTERNAL_IP ], &datum, &nbrec_table_classes[NBREC_TABLE_NAT]);
}
...
nbrec_nat_index_set_external_mac(const struct nbrec_nat *row, const char *external_mac)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_nat_columns[ NBREC_NAT_COL_EXTERNAL_MAC ], &datum, &nbrec_table_classes[NBREC_TABLE_NAT]);
}
...
nbrec_nat_index_set_logical_ip(const struct nbrec_nat *row, const char *logical_ip)
{
    ...
    key->string = CONST_CAST(char *, logical_ip);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_nat_columns[ NBREC_NAT_COL_LOGICAL_IP ], &datum, &nbrec_table_classes[NBREC_TABLE_NAT]);
}
...
nbrec_nat_index_set_logical_port(const struct nbrec_nat *row, const char *logical_port)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_nat_columns[ NBREC_NAT_COL_LOGICAL_PORT ], &datum, &nbrec_table_classes[NBREC_TABLE_NAT]);
}
...
nbrec_nat_index_set_type(const struct nbrec_nat *row, const char *type)
{
    ...
    key->string = CONST_CAST(char *, type);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_nat_columns[ NBREC_NAT_COL_TYPE ], &datum, &nbrec_table_classes[NBREC_TABLE_NAT]);
}
...
struct ovsdb_idl_column nbrec_nat_columns[NBREC_NAT_N_COLUMNS] = {
    [NBREC_NAT_COL_EXTERNAL_IDS] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = nbrec_nat_parse_external_ids,
         .unparse = nbrec_nat_unparse_external_ids,
    },
...
const struct nbrec_nb_global_table *
nbrec_nb_global_table_get(const struct ovsdb_idl *idl)
{
    return (const struct nbrec_nb_global_table *) idl;
}

const struct nbrec_nb_global *
nbrec_nb_global_table_first(const struct nbrec_nb_global_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_nb_global_first(idl);
}


const struct nbrec_nb_global *
nbrec_nb_global_table_track_get_first(const struct nbrec_nb_global_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return nbrec_nb_global_track_get_first(idl);
}
...
static void
nbrec_nb_global_parse_ipsec(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct nbrec_nb_global *row = nbrec_nb_global_cast(row_);
...
static void
nbrec_nb_global_parse_options(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct nbrec_nb_global *row = nbrec_nb_global_cast(row_);
    smap_init(&row->options);
    for (size_t i = 0; i < datum->n; i++) {
...
static void
nbrec_nb_global_unparse_ipsec(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}
...
static void
nbrec_nb_global_unparse_options(struct ovsdb_idl_row *row_)
{
    struct nbrec_nb_global *row = nbrec_nb_global_cast(row_);
    smap_destroy(&row->options);
}
...
/* Searches table "NB_Global" for a row with UUID 'uuid'.  Returns
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct nbrec_nb_global *
nbrec_nb_global_table_get_for_uuid(const struct nbrec_nb_global_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_nb_global_get_for_uuid(idl, uuid);
}
 ...
 *     nbrec_nb_global_set_ipsec()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * nbrec_nb_global_set_ipsec() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
nbrec_nb_global_verify_ipsec(const struct nbrec_nb_global *row)
{
    ovsdb_idl_txn_verify(&row->header_, &nbrec_nb_global_col_ipsec);
}
 ...
 *
 *   - If "options" has already been modified (with
 *     nbrec_nb_global_set_options()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * nbrec_nb_global_set_options() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
nbrec_nb_global_verify_options(const struct nbrec_nb_global *row)
{
    ovsdb_idl_txn_verify(&row->header_, &nbrec_nb_global_col_options);
}
...
/* Returns the "ipsec" column's value from the "NB_Global" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "ipsec" member in nbrec_nb_global. */
const struct ovsdb_datum *
nbrec_nb_global_get_ipsec(const struct nbrec_nb_global *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_BOOLEAN);
    return ovsdb_idl_read(&row->header_, &nbrec_nb_global_col_ipsec);
}
...
/* Returns the "options" column's value from the "NB_Global" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "options" member in nbrec_nb_global. */
const struct ovsdb_datum *
nbrec_nb_global_get_options(const struct nbrec_nb_global *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED,
	enum ovsdb_atomic_type value_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    ovs_assert(value_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &nbrec_nb_global_col_options);
}
...
/* Sets the "ipsec" column from the "NB_Global" table in 'row' to
 * 'ipsec'.
 *
 * The caller retains ownership of the arguments. */
void
nbrec_nb_global_set_ipsec(const struct nbrec_nb_global *row, bool ipsec)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    key.boolean = ipsec;
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &nbrec_nb_global_col_ipsec, &datum);
}
...
/* Sets the "options" column's value from the "NB_Global" table in 'row'
 * to 'options'.
 *
 * The caller retains ownership of 'options' and everything in it. */
void
nbrec_nb_global_set_options(const struct nbrec_nb_global *row, const struct smap *options)
{
    struct ovsdb_datum datum;
...
/* Sets an element of the "options" map column from the "NB_Global" table in 'row'
 * to 'new_value' given the key value 'new_key'.
 *
 */
void
nbrec_nb_global_update_options_setkey(const struct nbrec_nb_global *row, const char *new_key, const char *new_value)
{
    struct ovsdb_datum *datum;
...
/* Deletes an element of the "options" map column from the "NB_Global" table in 'row'
 * given the key value 'delete_key'.
 *
 */
void
nbrec_nb_global_update_options_delkey(const struct nbrec_nb_global *row, const char *delete_key)
{
    struct ovsdb_datum *datum;
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_nb_global_add_clause_ipsec(struct ovsdb_idl_condition *cond, enum ovsdb_function function, bool ipsec)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
 ...
 * The caller retains ownership of 'options' and everything in it. */
void
nbrec_nb_global_add_clause_options(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const struct smap *options)
{
    struct ovsdb_datum datum;
    ...
    ovsdb_datum_destroy(&datum, &nbrec_nb_global_col_options.type);
}
...
struct nbrec_nb_global *
nbrec_nb_global_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_nb_global);
    return ALIGNED_CAST(struct nbrec_nb_global *, ovsdb_idl_index_init_row(index));
}

struct nbrec_nb_global *
nbrec_nb_global_index_find(struct ovsdb_idl_index *index, const struct nbrec_nb_global *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_nb_global);
    return nbrec_nb_global_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
nbrec_nb_global_index_compare(
    struct ovsdb_idl_index *index, 
    const struct nbrec_nb_global *a, 
    const struct nbrec_nb_global *b)
{
    return ovsdb_idl_index_compare(index, &a->header_, &b->header_);
}

struct ovsdb_idl_cursor
nbrec_nb_global_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_nb_global);
    return ovsdb_idl_cursor_first(index);
}
...
struct ovsdb_idl_cursor
nbrec_nb_global_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct nbrec_nb_global *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_nb_global);
    return ovsdb_idl_cursor_first_eq(index, &target->header_);
}
...
struct ovsdb_idl_cursor
nbrec_nb_global_cursor_first_ge(
    struct ovsdb_idl_index *index, const struct nbrec_nb_global *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_nb_global);
    return ovsdb_idl_cursor_first_ge(index, &target->header_);
...
struct nbrec_nb_global *
nbrec_nb_global_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return nbrec_nb_global_cast(ovsdb_idl_cursor_data(cursor));
}
...
nbrec_nb_global_index_set_connections(const struct nbrec_nb_global *row, struct nbrec_connection **connections, size_t n_connections)
{
    ...
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_UUID, OVSDB_TYPE_VOID);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_nb_global_columns[ NBREC_NB_GLOBAL_COL_CONNECTIONS ], &datum, &nbrec_table_classes[NBREC_TABLE_NB_GLOBAL]);
}
...
nbrec_nb_global_index_set_hv_cfg(const struct nbrec_nb_global *row, int64_t hv_cfg)
{
    ...
    key->integer = hv_cfg;
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_nb_global_columns[ NBREC_NB_GLOBAL_COL_HV_CFG ], &datum, &nbrec_table_classes[NBREC_TABLE_NB_GLOBAL]);
}
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_nb_global_index_set_ipsec(const struct nbrec_nb_global *row, bool ipsec)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof(union ovsdb_atom));
    ...
    key->boolean = ipsec;
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_nb_global_columns[ NBREC_NB_GLOBAL_COL_IPSEC ], &datum, &nbrec_table_classes[NBREC_TABLE_NB_GLOBAL]);
}
...
nbrec_nb_global_index_set_nb_cfg(const struct nbrec_nb_global *row, int64_t nb_cfg)
{
    ...
    key->integer = nb_cfg;
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_nb_global_columns[ NBREC_NB_GLOBAL_COL_NB_CFG ], &datum, &nbrec_table_classes[NBREC_TABLE_NB_GLOBAL]);
}
 ...
 * The caller retains ownership of 'options' and everything in it. */
void
nbrec_nb_global_index_set_options(const struct nbrec_nb_global *row, const struct smap *options)
{
    struct ovsdb_datum *datum = xmalloc(sizeof(struct ovsdb_datum));
    ...
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_),
                          &nbrec_nb_global_columns[NBREC_NB_GLOBAL_COL_OPTIONS],
                          datum,
                          &nbrec_table_classes[NBREC_TABLE_NB_GLOBAL]);
}
...
nbrec_nb_global_index_set_sb_cfg(const struct nbrec_nb_global *row, int64_t sb_cfg)
{
    ...
    key->integer = sb_cfg;
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_nb_global_columns[ NBREC_NB_GLOBAL_COL_SB_CFG ], &datum, &nbrec_table_classes[NBREC_TABLE_NB_GLOBAL]);
}
...
nbrec_nb_global_index_set_ssl(const struct nbrec_nb_global *row, const struct nbrec_ssl *ssl)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_nb_global_columns[ NBREC_NB_GLOBAL_COL_SSL ], &datum, &nbrec_table_classes[NBREC_TABLE_NB_GLOBAL]);
}
...
struct ovsdb_idl_column nbrec_nb_global_columns[NBREC_NB_GLOBAL_N_COLUMNS] = {
    ...
    [NBREC_NB_GLOBAL_COL_IPSEC] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = nbrec_nb_global_parse_ipsec,
         .unparse = nbrec_nb_global_unparse_ipsec,
    },
    ...
    [NBREC_NB_GLOBAL_COL_OPTIONS] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = nbrec_nb_global_parse_options,
         .unparse = nbrec_nb_global_unparse_options,
    },
...
nbrec_port_group_table_get(const struct ovsdb_idl *idl)
{
    return (const struct nbrec_port_group_table *) idl;
}

const struct nbrec_port_group *
nbrec_port_group_table_first(const struct nbrec_port_group_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_port_group_first(idl);
}


const struct nbrec_port_group *
nbrec_port_group_table_track_get_first(const struct nbrec_port_group_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return nbrec_port_group_track_get_first(idl);
}


static void
nbrec_port_group_parse_acls(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct nbrec_port_group *row = nbrec_port_group_cast(row_);
    row->acls = NULL;
    row->n_acls = 0;
    for (size_t i = 0; i < datum->n; i++) {
        struct nbrec_acl *keyRow = nbrec_acl_cast(ovsdb_idl_get_row_arc(row_, &nbrec_table_acl, &datum->keys[i].uuid));
...
static void
nbrec_port_group_parse_external_ids(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct nbrec_port_group *row = nbrec_port_group_cast(row_);
    smap_init(&row->external_ids);
    for (size_t i = 0; i < datum->n; i++) {
...
static void
nbrec_port_group_parse_name(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct nbrec_port_group *row = nbrec_port_group_cast(row_);
...
static void
nbrec_port_group_parse_ports(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct nbrec_port_group *row = nbrec_port_group_cast(row_);
    row->ports = NULL;
    row->n_ports = 0;
...
static void
nbrec_port_group_unparse_acls(struct ovsdb_idl_row *row_)
{
    struct nbrec_port_group *row = nbrec_port_group_cast(row_);
    free(row->acls);
}
...
static void
nbrec_port_group_unparse_external_ids(struct ovsdb_idl_row *row_)
{
    struct nbrec_port_group *row = nbrec_port_group_cast(row_);
    smap_destroy(&row->external_ids);
}
...
static void
nbrec_port_group_unparse_name(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}

static void
nbrec_port_group_unparse_ports(struct ovsdb_idl_row *row_)
{
    struct nbrec_port_group *row = nbrec_port_group_cast(row_);
    free(row->ports);
}
...
static void
nbrec_port_group_init__(struct ovsdb_idl_row *row)
{
    nbrec_port_group_init(nbrec_port_group_cast(row));
}

/* Clears the contents of 'row' in table "Port_Group". */
void
nbrec_port_group_init(struct nbrec_port_group *row)
{
    memset(row, 0, sizeof *row); 
    smap_init(&row->external_ids);
    row->name = "";
 ...
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct nbrec_port_group *
nbrec_port_group_get_for_uuid(const struct ovsdb_idl *idl, const struct uuid *uuid)
{
    return nbrec_port_group_cast(ovsdb_idl_get_row_for_uuid(idl, &nbrec_table_port_group, uuid));
}

/* Searches table "Port_Group" for a row with UUID 'uuid'.  Returns
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct nbrec_port_group *
nbrec_port_group_table_get_for_uuid(const struct nbrec_port_group_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_port_group_get_for_uuid(idl, uuid);
}
 ...
 * visited twice or not at apply. */
const struct nbrec_port_group *
nbrec_port_group_first(const struct ovsdb_idl *idl)
{
    return nbrec_port_group_cast(ovsdb_idl_first_row(idl, &nbrec_table_port_group));
}

/* Returns a row following 'row' within its table, or a null pointer if 'row'
 * is the last row in its table. */
const struct nbrec_port_group *
nbrec_port_group_next(const struct nbrec_port_group *row)
{
    return nbrec_port_group_cast(ovsdb_idl_next_row(&row->header_));
}

unsigned int nbrec_port_group_get_seqno(const struct ovsdb_idl *idl)
{
    return ovsdb_idl_table_get_seqno(idl, &nbrec_table_port_group);
}

unsigned int nbrec_port_group_row_get_seqno(const struct nbrec_port_group *row, enum ovsdb_idl_change change)
{
    return ovsdb_idl_row_get_seqno(&row->header_, change);
}

const struct nbrec_port_group *
nbrec_port_group_track_get_first(const struct ovsdb_idl *idl)
{
    return nbrec_port_group_cast(ovsdb_idl_track_get_first(idl, &nbrec_table_port_group));
}

const struct nbrec_port_group
*nbrec_port_group_track_get_next(const struct nbrec_port_group *row)
{
    return nbrec_port_group_cast(ovsdb_idl_track_get_next(&row->header_));
}
 ...
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
nbrec_port_group_delete(const struct nbrec_port_group *row)
{
    ovsdb_idl_txn_delete(&row->header_);
}
 ...
 * but the IDL will replace any uses of the provisional UUID in the
 * data to be to be committed by the UUID assigned by ovsdb-server. */
struct nbrec_port_group *
nbrec_port_group_insert(struct ovsdb_idl_txn *txn)
{
    return nbrec_port_group_cast(ovsdb_idl_txn_insert(txn, &nbrec_table_port_group, NULL));
}

bool
nbrec_port_group_is_updated(const struct nbrec_port_group *row, enum nbrec_port_group_column_id column)
{
    return ovsdb_idl_track_is_updated(&row->header_, &nbrec_port_group_columns[column]);
}
 ...
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     nbrec_port_group_insert()).
 *
 *   - If "acls" has already been modified (with
 *     nbrec_port_group_set_acls()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * nbrec_port_group_set_acls() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
nbrec_port_group_verify_acls(const struct nbrec_port_group *row)
{
    ovsdb_idl_txn_verify(&row->header_, &nbrec_port_group_col_acls);
}
 ...
 *
 *   - If "external_ids" has already been modified (with
 *     nbrec_port_group_set_external_ids()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * nbrec_port_group_set_external_ids() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
nbrec_port_group_verify_external_ids(const struct nbrec_port_group *row)
{
    ovsdb_idl_txn_verify(&row->header_, &nbrec_port_group_col_external_ids);
}
 ...
 *
 *   - If "name" has already been modified (with
 *     nbrec_port_group_set_name()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * nbrec_port_group_set_name() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
nbrec_port_group_verify_name(const struct nbrec_port_group *row)
{
    ovsdb_idl_txn_verify(&row->header_, &nbrec_port_group_col_name);
}
 ...
 *
 *   - If "ports" has already been modified (with
 *     nbrec_port_group_set_ports()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * nbrec_port_group_set_ports() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
nbrec_port_group_verify_ports(const struct nbrec_port_group *row)
{
    ovsdb_idl_txn_verify(&row->header_, &nbrec_port_group_col_ports);
}

/* Returns the "acls" column's value from the "Port_Group" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "acls" member in nbrec_port_group. */
const struct ovsdb_datum *
nbrec_port_group_get_acls(const struct nbrec_port_group *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_UUID);
    return ovsdb_idl_read(&row->header_, &nbrec_port_group_col_acls);
}

/* Returns the "external_ids" column's value from the "Port_Group" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "external_ids" member in nbrec_port_group. */
const struct ovsdb_datum *
nbrec_port_group_get_external_ids(const struct nbrec_port_group *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED,
	enum ovsdb_atomic_type value_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    ovs_assert(value_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &nbrec_port_group_col_external_ids);
}
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "name" member in nbrec_port_group. */
const struct ovsdb_datum *
nbrec_port_group_get_name(const struct nbrec_port_group *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &nbrec_port_group_col_name);
}
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "ports" member in nbrec_port_group. */
const struct ovsdb_datum *
nbrec_port_group_get_ports(const struct nbrec_port_group *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_UUID);
    return ovsdb_idl_read(&row->header_, &nbrec_port_group_col_ports);
}

/* Sets the "acls" column from the "Port_Group" table in 'row' to
 * the 'acls' set with 'n_acls' entries.
 *
 * The caller retains ownership of the arguments. */
void
nbrec_port_group_set_acls(const struct nbrec_port_group *row, struct nbrec_acl **acls, size_t n_acls)
{
    struct ovsdb_datum datum;
    ...
    ovsdb_idl_txn_write(&row->header_, &nbrec_port_group_col_acls, &datum);
}

/* Sets the "external_ids" column's value from the "Port_Group" table in 'row'
 * to 'external_ids'.
 *
 * The caller retains ownership of 'external_ids' and everything in it. */
void
nbrec_port_group_set_external_ids(const struct nbrec_port_group *row, const struct smap *external_ids)
{
    struct ovsdb_datum datum;
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_port_group_set_name(const struct nbrec_port_group *row, const char *name)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    key.string = CONST_CAST(char *, name);
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &nbrec_port_group_col_name, &datum);
}
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_port_group_set_ports(const struct nbrec_port_group *row, struct nbrec_logical_switch_port **ports, size_t n_ports)
{
    struct ovsdb_datum datum;
    ...
    ovsdb_idl_txn_write(&row->header_, &nbrec_port_group_col_ports, &datum);
}

/* Adds the value 'new_value' to the "acls" set column from the "Port_Group" table
 * in 'row'.
 *
 */
void
nbrec_port_group_update_acls_addvalue(const struct nbrec_port_group *row, const struct nbrec_acl *new_value)
{
    struct ovsdb_datum *datum;
 ...
 */
void
nbrec_port_group_update_acls_delvalue(const struct nbrec_port_group *row, const struct nbrec_acl *delete_value)
{
    struct ovsdb_datum *datum;
...
/* Sets an element of the "external_ids" map column from the "Port_Group" table in 'row'
 * to 'new_value' given the key value 'new_key'.
 *
 */
void
nbrec_port_group_update_external_ids_setkey(const struct nbrec_port_group *row, const char *new_key, const char *new_value)
{
    struct ovsdb_datum *datum;
...
/* Deletes an element of the "external_ids" map column from the "Port_Group" table in 'row'
 * given the key value 'delete_key'.
 *
 */
void
nbrec_port_group_update_external_ids_delkey(const struct nbrec_port_group *row, const char *delete_key)
{
    struct ovsdb_datum *datum;
 ...
 */
void
nbrec_port_group_update_ports_addvalue(const struct nbrec_port_group *row, const struct nbrec_logical_switch_port *new_value)
{
    struct ovsdb_datum *datum;
 ...
 */
void
nbrec_port_group_update_ports_delvalue(const struct nbrec_port_group *row, const struct nbrec_logical_switch_port *delete_value)
{
    struct ovsdb_datum *datum;
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_port_group_add_clause_acls(struct ovsdb_idl_condition *cond, enum ovsdb_function function, struct uuid **acls, size_t n_acls)
{
    struct ovsdb_datum datum;
    datum.n = n_acls;
    datum.keys = n_acls ? xmalloc(n_acls * sizeof *datum.keys) : NULL;
 ...
 * The caller retains ownership of 'external_ids' and everything in it. */
void
nbrec_port_group_add_clause_external_ids(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const struct smap *external_ids)
{
    struct ovsdb_datum datum;
    ...
    ovsdb_datum_destroy(&datum, &nbrec_port_group_col_external_ids.type);
}
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_port_group_add_clause_name(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const char *name)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_port_group_add_clause_ports(struct ovsdb_idl_condition *cond, enum ovsdb_function function, struct uuid **ports, size_t n_ports)
{
    struct ovsdb_datum datum;
    datum.n = n_ports;
    datum.keys = n_ports ? xmalloc(n_ports * sizeof *datum.keys) : NULL;
 ...
 */
void
nbrec_port_group_index_destroy_row(const struct nbrec_port_group *row)
{
    ovsdb_idl_index_destroy_row(&row->header_);
}
...
struct nbrec_port_group *
nbrec_port_group_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_port_group);
    return ALIGNED_CAST(struct nbrec_port_group *, ovsdb_idl_index_init_row(index));
}

struct nbrec_port_group *
nbrec_port_group_index_find(struct ovsdb_idl_index *index, const struct nbrec_port_group *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_port_group);
    return nbrec_port_group_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
nbrec_port_group_index_compare(
    struct ovsdb_idl_index *index, 
    const struct nbrec_port_group *a, 
    const struct nbrec_port_group *b)
{
    return ovsdb_idl_index_compare(index, &a->header_, &b->header_);
}

struct ovsdb_idl_cursor
nbrec_port_group_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_port_group);
    return ovsdb_idl_cursor_first(index);
}
...
struct ovsdb_idl_cursor
nbrec_port_group_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct nbrec_port_group *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_port_group);
    return ovsdb_idl_cursor_first_eq(index, &target->header_);
}
...
struct ovsdb_idl_cursor
nbrec_port_group_cursor_first_ge(
    struct ovsdb_idl_index *index, const struct nbrec_port_group *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_port_group);
    return ovsdb_idl_cursor_first_ge(index, &target->header_);
...
struct nbrec_port_group *
nbrec_port_group_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return nbrec_port_group_cast(ovsdb_idl_cursor_data(cursor));
}
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_port_group_index_set_acls(const struct nbrec_port_group *row, struct nbrec_acl **acls, size_t n_acls)
{
    struct ovsdb_datum datum;
    size_t i;
    ...
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_UUID, OVSDB_TYPE_VOID);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_port_group_columns[ NBREC_PORT_GROUP_COL_ACLS ], &datum, &nbrec_table_classes[NBREC_TABLE_PORT_GROUP]);
}
 ...
 * The caller retains ownership of 'external_ids' and everything in it. */
void
nbrec_port_group_index_set_external_ids(const struct nbrec_port_group *row, const struct smap *external_ids)
{
    struct ovsdb_datum *datum = xmalloc(sizeof(struct ovsdb_datum));
    ...
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_),
                          &nbrec_port_group_columns[NBREC_PORT_GROUP_COL_EXTERNAL_IDS],
                          datum,
                          &nbrec_table_classes[NBREC_TABLE_PORT_GROUP]);
}
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_port_group_index_set_name(const struct nbrec_port_group *row, const char *name)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof(union ovsdb_atom));
    ...
    key->string = CONST_CAST(char *, name);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_port_group_columns[ NBREC_PORT_GROUP_COL_NAME ], &datum, &nbrec_table_classes[NBREC_TABLE_PORT_GROUP]);
}
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_port_group_index_set_ports(const struct nbrec_port_group *row, struct nbrec_logical_switch_port **ports, size_t n_ports)
{
    struct ovsdb_datum datum;
    size_t i;
    ...
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_UUID, OVSDB_TYPE_VOID);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_port_group_columns[ NBREC_PORT_GROUP_COL_PORTS ], &datum, &nbrec_table_classes[NBREC_TABLE_PORT_GROUP]);
}

struct ovsdb_idl_column nbrec_port_group_columns[NBREC_PORT_GROUP_N_COLUMNS];

void
nbrec_port_group_set_condition(struct ovsdb_idl *idl, struct ovsdb_idl_condition *condition)
{
    ovsdb_idl_set_condition(idl, &nbrec_table_port_group, condition);
}
...
struct ovsdb_idl_column nbrec_port_group_columns[NBREC_PORT_GROUP_N_COLUMNS] = {
    [NBREC_PORT_GROUP_COL_ACLS] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = nbrec_port_group_parse_acls,
         .unparse = nbrec_port_group_unparse_acls,
    },
    ...
    [NBREC_PORT_GROUP_COL_EXTERNAL_IDS] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = nbrec_port_group_parse_external_ids,
         .unparse = nbrec_port_group_unparse_external_ids,
    },
    ...
    [NBREC_PORT_GROUP_COL_NAME] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = nbrec_port_group_parse_name,
         .unparse = nbrec_port_group_unparse_name,
    },
    ...
    [NBREC_PORT_GROUP_COL_PORTS] = {
         ...
         .type = {
            .key = {
               .type = OVSDB_TYPE_UUID,
               .uuid = { .refTableName = "Logical_Switch_Port", .refType = OVSDB_REF_WEAK },
            },
            .value = OVSDB_BASE_VOID_INIT,
            .n_min = 0,
            .n_max = UINT_MAX,
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = nbrec_port_group_parse_ports,
         .unparse = nbrec_port_group_unparse_ports,
    },
...
const struct nbrec_qos_table *
nbrec_qos_table_get(const struct ovsdb_idl *idl)
{
    return (const struct nbrec_qos_table *) idl;
}

const struct nbrec_qos *
nbrec_qos_table_first(const struct nbrec_qos_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_qos_first(idl);
}


const struct nbrec_qos *
nbrec_qos_table_track_get_first(const struct nbrec_qos_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return nbrec_qos_track_get_first(idl);
}
...
nbrec_qos_parse_action(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    ...
    row->key_action = NULL;
    row->value_action = NULL;
    row->n_action = 0;
    for (size_t i = 0; i < datum->n; i++) {
        if (!row->n_action) {
            row->key_action = xmalloc(datum->n * sizeof *row->key_action);
            row->value_action = xmalloc(datum->n * sizeof *row->value_action);
        }
        row->key_action[row->n_action] = datum->keys[i].string;
        row->value_action[row->n_action] = datum->values[i].integer;
        row->n_action++;
    }
}

static void
nbrec_qos_parse_bandwidth(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct nbrec_qos *row = nbrec_qos_cast(row_);
    row->key_bandwidth = NULL;
    row->value_bandwidth = NULL;
    row->n_bandwidth = 0;
    for (size_t i = 0; i < datum->n; i++) {
        if (!row->n_bandwidth) {
            row->key_bandwidth = xmalloc(datum->n * sizeof *row->key_bandwidth);
            row->value_bandwidth = xmalloc(datum->n * sizeof *row->value_bandwidth);
        }
        row->key_bandwidth[row->n_bandwidth] = datum->keys[i].string;
        row->value_bandwidth[row->n_bandwidth] = datum->values[i].integer;
        row->n_bandwidth++;
    }
}
...
static void
nbrec_qos_unparse_action(struct ovsdb_idl_row *row_)
{
    struct nbrec_qos *row = nbrec_qos_cast(row_);
    free(row->key_action);
    free(row->value_action);
}

static void
nbrec_qos_unparse_bandwidth(struct ovsdb_idl_row *row_)
{
    struct nbrec_qos *row = nbrec_qos_cast(row_);
    free(row->key_bandwidth);
    free(row->value_bandwidth);
}
 ...
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct nbrec_qos *
nbrec_qos_table_get_for_uuid(const struct nbrec_qos_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_qos_get_for_uuid(idl, uuid);
}
...
/* Causes the original contents of column "bandwidth" in 'row' to be
 * verified as a prerequisite to completing the transaction.  That is, if
 * "bandwidth" in 'row' changed (or if 'row' was deleted) between the
 * time that the IDL originally read its contents and the time that the
 * transaction aborts and ovsdb_idl_txn_commit() returns TXN_TRY_AGAIN.
 *
 * The intention is that, to ensure that no transaction commits based on dirty
 * reads, an application should call this function any time "bandwidth" is
 * read as part of a read-modify-write operation.
 *
 * In some cases this function reduces to a no-op, because the current value
 * of "bandwidth" is already known:
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     nbrec_qos_insert()).
 *
 *   - If "bandwidth" has already been modified (with
 *     nbrec_qos_set_bandwidth()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * nbrec_qos_set_bandwidth() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
nbrec_qos_verify_bandwidth(const struct nbrec_qos *row)
{
    ovsdb_idl_txn_verify(&row->header_, &nbrec_qos_col_bandwidth);
}
...
/* Returns the "bandwidth" column's value from the "QoS" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 ...
 * 'key_type' must be OVSDB_TYPE_STRING.
 * 'value_type' must be OVSDB_TYPE_INTEGER.
 * (This helps to avoid silent bugs if someone changes bandwidth's
 * type without updating the caller.)
 *
 * The caller must not modify or free the returned value.
 *
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "bandwidth" member in nbrec_qos. */
const struct ovsdb_datum *
nbrec_qos_get_bandwidth(const struct nbrec_qos *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED,
	enum ovsdb_atomic_type value_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    ovs_assert(value_type == OVSDB_TYPE_INTEGER);
    return ovsdb_idl_read(&row->header_, &nbrec_qos_col_bandwidth);
}
 ...
 * the map with keys 'key_action' and values 'value_action'
 * with 'n_action' entries.
 *
 * Argument constraints: key must be "dscp", value in range 0 to 63
 *
 * The caller retains ownership of the arguments. */
void
nbrec_qos_set_action(const struct nbrec_qos *row, const char **key_action, const int64_t *value_action, size_t n_action)
{
    struct ovsdb_datum datum;

    datum.n = n_action;
    datum.keys = n_action ? xmalloc(n_action * sizeof *datum.keys) : NULL;
    datum.values = xmalloc(n_action * sizeof *datum.values);
    for (size_t i = 0; i < n_action; i++) {
        datum.keys[i].string = xstrdup(key_action[i]);
        datum.values[i].integer = value_action[i];
    }
    ovsdb_idl_txn_write(&row->header_, &nbrec_qos_col_action, &datum);
}

/* Sets the "bandwidth" column from the "QoS" table in 'row' to
 * the map with keys 'key_bandwidth' and values 'value_bandwidth'
 * with 'n_bandwidth' entries.
 *
 * Argument constraints: key either "burst" or "rate", value in range 1 to 4,294,967,295
 *
 * The caller retains ownership of the arguments. */
void
nbrec_qos_set_bandwidth(const struct nbrec_qos *row, const char **key_bandwidth, const int64_t *value_bandwidth, size_t n_bandwidth)
{
    struct ovsdb_datum datum;

    datum.n = n_bandwidth;
    datum.keys = n_bandwidth ? xmalloc(n_bandwidth * sizeof *datum.keys) : NULL;
    datum.values = xmalloc(n_bandwidth * sizeof *datum.values);
    for (size_t i = 0; i < n_bandwidth; i++) {
        datum.keys[i].string = xstrdup(key_bandwidth[i]);
        datum.values[i].integer = value_bandwidth[i];
    }
    ovsdb_idl_txn_write(&row->header_, &nbrec_qos_col_bandwidth, &datum);
}
...
/* Sets an element of the "bandwidth" map column from the "QoS" table in 'row'
 * to 'new_value' given the key value 'new_key'.
 *
 */
void
nbrec_qos_update_bandwidth_setkey(const struct nbrec_qos *row, const char *new_key, int64_t new_value)
{
    struct ovsdb_datum *datum;
    ...
    ovsdb_idl_txn_write_partial_map(&row->header_,
                                    &nbrec_qos_col_bandwidth,
                                    datum);
}

/* Deletes an element of the "bandwidth" map column from the "QoS" table in 'row'
 * given the key value 'delete_key'.
 *
 */
void
nbrec_qos_update_bandwidth_delkey(const struct nbrec_qos *row, const char *delete_key)
{
    struct ovsdb_datum *datum;
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_qos_add_clause_action(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const char **key_action, const int64_t *value_action, size_t n_action)
{
    struct ovsdb_datum datum;
    datum.n = n_action;
    datum.keys = n_action ? xmalloc(n_action * sizeof *datum.keys) : NULL;
    datum.values = xmalloc(n_action * sizeof *datum.values);
    for (size_t i = 0; i < n_action; i++) {
        datum.keys[i].string = CONST_CAST(char *, key_action[i]);
        datum.values[i].integer = value_action[i];
    }
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_qos_add_clause_bandwidth(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const char **key_bandwidth, const int64_t *value_bandwidth, size_t n_bandwidth)
{
    struct ovsdb_datum datum;
    datum.n = n_bandwidth;
    datum.keys = n_bandwidth ? xmalloc(n_bandwidth * sizeof *datum.keys) : NULL;
    datum.values = xmalloc(n_bandwidth * sizeof *datum.values);
    for (size_t i = 0; i < n_bandwidth; i++) {
        datum.keys[i].string = CONST_CAST(char *, key_bandwidth[i]);
        datum.values[i].integer = value_bandwidth[i];
    }
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_STRING, OVSDB_TYPE_INTEGER);
    ovsdb_idl_condition_add_clause(cond,
...
struct nbrec_qos *
nbrec_qos_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_qos);
    return ALIGNED_CAST(struct nbrec_qos *, ovsdb_idl_index_init_row(index));
}

struct nbrec_qos *
nbrec_qos_index_find(struct ovsdb_idl_index *index, const struct nbrec_qos *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_qos);
    return nbrec_qos_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
nbrec_qos_index_compare(
    struct ovsdb_idl_index *index, 
    const struct nbrec_qos *a, 
    const struct nbrec_qos *b)
{
    return ovsdb_idl_index_compare(index, &a->header_, &b->header_);
}

struct ovsdb_idl_cursor
nbrec_qos_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_qos);
    return ovsdb_idl_cursor_first(index);
}
...
struct ovsdb_idl_cursor
nbrec_qos_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct nbrec_qos *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_qos);
    return ovsdb_idl_cursor_first_eq(index, &target->header_);
}
...
struct nbrec_qos *
nbrec_qos_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return nbrec_qos_cast(ovsdb_idl_cursor_data(cursor));
}
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_qos_index_set_action(const struct nbrec_qos *row, const char **key_action, const int64_t *value_action, size_t n_action)
{
    struct ovsdb_datum datum;
    size_t i;
    ...
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_STRING, OVSDB_TYPE_INTEGER);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_qos_columns[ NBREC_QOS_COL_ACTION ], &datum, &nbrec_table_classes[NBREC_TABLE_QOS]);
}
 ...
 * The caller retains ownership of the arguments. */
void
nbrec_qos_index_set_bandwidth(const struct nbrec_qos *row, const char **key_bandwidth, const int64_t *value_bandwidth, size_t n_bandwidth)
{
    struct ovsdb_datum datum;
    size_t i;
    ...
    datum.keys = n_bandwidth ? xmalloc(n_bandwidth * sizeof *datum.keys) : NULL;
    datum.values = xmalloc(n_bandwidth * sizeof *datum.values);
    for (i = 0; i < n_bandwidth; i++) {
        datum.keys[i].string = xstrdup(key_bandwidth[i]);
        datum.values[i].integer = value_bandwidth[i];
    }
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_STRING, OVSDB_TYPE_INTEGER);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_qos_columns[ NBREC_QOS_COL_BANDWIDTH ], &datum, &nbrec_table_classes[NBREC_TABLE_QOS]);
}
...
nbrec_qos_index_set_direction(const struct nbrec_qos *row, const char *direction)
{
    ...
    key->string = CONST_CAST(char *, direction);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_qos_columns[ NBREC_QOS_COL_DIRECTION ], &datum, &nbrec_table_classes[NBREC_TABLE_QOS]);
}
...
nbrec_qos_index_set_match(const struct nbrec_qos *row, const char *match)
{
    ...
    key->string = CONST_CAST(char *, match);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_qos_columns[ NBREC_QOS_COL_MATCH ], &datum, &nbrec_table_classes[NBREC_TABLE_QOS]);
}
...
nbrec_qos_index_set_priority(const struct nbrec_qos *row, int64_t priority)
{
    ...
    key->integer = priority;
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_qos_columns[ NBREC_QOS_COL_PRIORITY ], &datum, &nbrec_table_classes[NBREC_TABLE_QOS]);
}
...
static union ovsdb_atom QoS_col_bandwidth_key_enum_keys[2] = {
    { .string = "burst" },
    { .string = "rate" },
};
static struct ovsdb_datum QoS_col_bandwidth_key_enum = {
    .n = 2,
    .keys = QoS_col_bandwidth_key_enum_keys,
};
...
struct ovsdb_idl_column nbrec_qos_columns[NBREC_QOS_N_COLUMNS] = {
    ...
    [NBREC_QOS_COL_BANDWIDTH] = {
         ...
         .type = {
            .key = {
               .type = OVSDB_TYPE_STRING,
               .enum_ = &QoS_col_bandwidth_key_enum,
               .string = { .minLen = 0, .maxLen = UINT_MAX },
            },
            .value = {
                .type = OVSDB_TYPE_INTEGER,
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = nbrec_qos_parse_bandwidth,
         .unparse = nbrec_qos_unparse_bandwidth,
    },
...
const struct nbrec_ssl_table *
nbrec_ssl_table_get(const struct ovsdb_idl *idl)
{
    return (const struct nbrec_ssl_table *) idl;
}

const struct nbrec_ssl *
nbrec_ssl_table_first(const struct nbrec_ssl_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_ssl_first(idl);
}


const struct nbrec_ssl *
nbrec_ssl_table_track_get_first(const struct nbrec_ssl_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return nbrec_ssl_track_get_first(idl);
}
 ...
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct nbrec_ssl *
nbrec_ssl_table_get_for_uuid(const struct nbrec_ssl_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return nbrec_ssl_get_for_uuid(idl, uuid);
}
...
struct nbrec_ssl *
nbrec_ssl_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_ssl);
    return ALIGNED_CAST(struct nbrec_ssl *, ovsdb_idl_index_init_row(index));
}

struct nbrec_ssl *
nbrec_ssl_index_find(struct ovsdb_idl_index *index, const struct nbrec_ssl *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_ssl);
    return nbrec_ssl_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
nbrec_ssl_index_compare(
    struct ovsdb_idl_index *index, 
    const struct nbrec_ssl *a, 
    const struct nbrec_ssl *b)
{
    return ovsdb_idl_index_compare(index, &a->header_, &b->header_);
}

struct ovsdb_idl_cursor
nbrec_ssl_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &nbrec_table_ssl);
    return ovsdb_idl_cursor_first(index);
}
...
struct ovsdb_idl_cursor
nbrec_ssl_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct nbrec_ssl *target)
{
    ovs_assert(index->table->class_ == &nbrec_table_ssl);
    return ovsdb_idl_cursor_first_eq(index, &target->header_);
}
...
struct nbrec_ssl *
nbrec_ssl_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return nbrec_ssl_cast(ovsdb_idl_cursor_data(cursor));
}
...
nbrec_ssl_index_set_bootstrap_ca_cert(const struct nbrec_ssl *row, bool bootstrap_ca_cert)
{
    ...
    key->boolean = bootstrap_ca_cert;
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_ssl_columns[ NBREC_SSL_COL_BOOTSTRAP_CA_CERT ], &datum, &nbrec_table_classes[NBREC_TABLE_SSL]);
}
...
nbrec_ssl_index_set_ca_cert(const struct nbrec_ssl *row, const char *ca_cert)
{
    ...
    key->string = CONST_CAST(char *, ca_cert);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_ssl_columns[ NBREC_SSL_COL_CA_CERT ], &datum, &nbrec_table_classes[NBREC_TABLE_SSL]);
}
...
nbrec_ssl_index_set_certificate(const struct nbrec_ssl *row, const char *certificate)
{
    ...
    key->string = CONST_CAST(char *, certificate);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_ssl_columns[ NBREC_SSL_COL_CERTIFICATE ], &datum, &nbrec_table_classes[NBREC_TABLE_SSL]);
}
...
nbrec_ssl_index_set_private_key(const struct nbrec_ssl *row, const char *private_key)
{
    ...
    key->string = CONST_CAST(char *, private_key);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_ssl_columns[ NBREC_SSL_COL_PRIVATE_KEY ], &datum, &nbrec_table_classes[NBREC_TABLE_SSL]);
}
...
nbrec_ssl_index_set_ssl_ciphers(const struct nbrec_ssl *row, const char *ssl_ciphers)
{
    ...
    key->string = CONST_CAST(char *, ssl_ciphers);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_ssl_columns[ NBREC_SSL_COL_SSL_CIPHERS ], &datum, &nbrec_table_classes[NBREC_TABLE_SSL]);
}
...
nbrec_ssl_index_set_ssl_protocols(const struct nbrec_ssl *row, const char *ssl_protocols)
{
    ...
    key->string = CONST_CAST(char *, ssl_protocols);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &nbrec_ssl_columns[ NBREC_SSL_COL_SSL_PROTOCOLS ], &datum, &nbrec_table_classes[NBREC_TABLE_SSL]);
}
...
struct ovsdb_idl_table_class nbrec_table_classes[NBREC_N_TABLES] = {
    ...
    {"Logical_Router_Port", false, false,
    ...
    {"Logical_Router_Static_Route", false, false,
    ...
    {"Logical_Switch_Port", false, false,
    ...
    {"Meter", true, false,
     nbrec_meter_columns, ARRAY_SIZE(nbrec_meter_columns),
     sizeof(struct nbrec_meter), nbrec_meter_init__},
    {"Meter_Band", false, false,
     nbrec_meter_band_columns, ARRAY_SIZE(nbrec_meter_band_columns),
     sizeof(struct nbrec_meter_band), nbrec_meter_band_init__},
    {"NAT", false, false,
    ...
    {"Port_Group", true, false,
     nbrec_port_group_columns, ARRAY_SIZE(nbrec_port_group_columns),
     sizeof(struct nbrec_port_group), nbrec_port_group_init__},
    {"QoS", false, false,
