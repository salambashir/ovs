...
#define nbrec_acl_col_meter (nbrec_acl_columns[NBREC_ACL_COL_METER])
...
const struct nbrec_acl_table *nbrec_acl_table_get(const struct ovsdb_idl *);
const struct nbrec_acl *nbrec_acl_table_first(const struct nbrec_acl_table *);

#define NBREC_ACL_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = nbrec_acl_table_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_acl_next(ROW))
#define NBREC_ACL_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = nbrec_acl_table_first(TABLE); \
             (ROW) ? ((NEXT) = nbrec_acl_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

const struct nbrec_acl *nbrec_acl_get_for_uuid(const struct ovsdb_idl *, const struct uuid *);
const struct nbrec_acl *nbrec_acl_table_get_for_uuid(const struct nbrec_acl_table *, const struct uuid *);
...
const struct nbrec_acl *nbrec_acl_table_track_get_first(const struct nbrec_acl_table *);
#define NBREC_ACL_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = nbrec_acl_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_acl_track_get_next(ROW))
...
struct nbrec_acl *nbrec_acl_index_find(struct ovsdb_idl_index *, const struct nbrec_acl *);

int nbrec_acl_index_compare(
    struct ovsdb_idl_index *, 
    const struct nbrec_acl *, 
    const struct nbrec_acl *);
struct ovsdb_idl_cursor nbrec_acl_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor nbrec_acl_cursor_first_eq(
    struct ovsdb_idl_index *, const struct nbrec_acl *);
struct ovsdb_idl_cursor nbrec_acl_cursor_first_ge(
    struct ovsdb_idl_index *, const struct nbrec_acl *);

struct nbrec_acl *nbrec_acl_cursor_data(struct ovsdb_idl_cursor *);

#define NBREC_ACL_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_acl_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = nbrec_acl_cursor_data(&cursor__), \
                  !(TO) || nbrec_acl_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define NBREC_ACL_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_acl_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = nbrec_acl_cursor_data(&cursor__), \
...
#define NBREC_ACL_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_acl_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = nbrec_acl_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
void nbrec_acl_verify_meter(const struct nbrec_acl *);
...
const struct ovsdb_datum *nbrec_acl_get_meter(const struct nbrec_acl *, enum ovsdb_atomic_type key_type);
...
void nbrec_acl_set_meter(const struct nbrec_acl *, const char *meter);
...
void nbrec_acl_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_acl_update_meter_addvalue(const struct nbrec_acl *,  const char *);
void nbrec_acl_update_meter_delvalue(const struct nbrec_acl *,  const char *);
void nbrec_acl_add_clause_meter(struct ovsdb_idl_condition *, enum ovsdb_function function, const char *meter);
void nbrec_acl_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
...
struct nbrec_acl *nbrec_acl_index_init_row(struct ovsdb_idl_index *);
...
const char *match);
void nbrec_acl_index_set_meter(const struct nbrec_acl *,
const char *meter);
...
const struct nbrec_address_set_table *nbrec_address_set_table_get(const struct ovsdb_idl *);
const struct nbrec_address_set *nbrec_address_set_table_first(const struct nbrec_address_set_table *);

#define NBREC_ADDRESS_SET_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = nbrec_address_set_table_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_address_set_next(ROW))
#define NBREC_ADDRESS_SET_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = nbrec_address_set_table_first(TABLE); \
...
const struct nbrec_address_set *nbrec_address_set_table_get_for_uuid(const struct nbrec_address_set_table *, const struct uuid *);
...
const struct nbrec_address_set *nbrec_address_set_table_track_get_first(const struct nbrec_address_set_table *);
#define NBREC_ADDRESS_SET_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = nbrec_address_set_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_address_set_track_get_next(ROW))
...
struct nbrec_address_set *nbrec_address_set_index_find(struct ovsdb_idl_index *, const struct nbrec_address_set *);

int nbrec_address_set_index_compare(
    struct ovsdb_idl_index *, 
    const struct nbrec_address_set *, 
    const struct nbrec_address_set *);
struct ovsdb_idl_cursor nbrec_address_set_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor nbrec_address_set_cursor_first_eq(
    struct ovsdb_idl_index *, const struct nbrec_address_set *);
struct ovsdb_idl_cursor nbrec_address_set_cursor_first_ge(
    struct ovsdb_idl_index *, const struct nbrec_address_set *);

struct nbrec_address_set *nbrec_address_set_cursor_data(struct ovsdb_idl_cursor *);

#define NBREC_ADDRESS_SET_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_address_set_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = nbrec_address_set_cursor_data(&cursor__), \
                  !(TO) || nbrec_address_set_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define NBREC_ADDRESS_SET_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_address_set_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = nbrec_address_set_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define NBREC_ADDRESS_SET_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_address_set_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = nbrec_address_set_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
struct nbrec_address_set *nbrec_address_set_index_init_row(struct ovsdb_idl_index *);
...
const struct nbrec_connection_table *nbrec_connection_table_get(const struct ovsdb_idl *);
const struct nbrec_connection *nbrec_connection_table_first(const struct nbrec_connection_table *);

#define NBREC_CONNECTION_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = nbrec_connection_table_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_connection_next(ROW))
#define NBREC_CONNECTION_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = nbrec_connection_table_first(TABLE); \
...
const struct nbrec_connection *nbrec_connection_table_get_for_uuid(const struct nbrec_connection_table *, const struct uuid *);
...
const struct nbrec_connection *nbrec_connection_table_track_get_first(const struct nbrec_connection_table *);
#define NBREC_CONNECTION_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = nbrec_connection_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_connection_track_get_next(ROW))
...
struct nbrec_connection *nbrec_connection_index_find(struct ovsdb_idl_index *, const struct nbrec_connection *);

int nbrec_connection_index_compare(
    struct ovsdb_idl_index *, 
    const struct nbrec_connection *, 
    const struct nbrec_connection *);
struct ovsdb_idl_cursor nbrec_connection_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor nbrec_connection_cursor_first_eq(
    struct ovsdb_idl_index *, const struct nbrec_connection *);
struct ovsdb_idl_cursor nbrec_connection_cursor_first_ge(
    struct ovsdb_idl_index *, const struct nbrec_connection *);

struct nbrec_connection *nbrec_connection_cursor_data(struct ovsdb_idl_cursor *);

#define NBREC_CONNECTION_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_connection_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = nbrec_connection_cursor_data(&cursor__), \
                  !(TO) || nbrec_connection_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define NBREC_CONNECTION_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_connection_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = nbrec_connection_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define NBREC_CONNECTION_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_connection_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = nbrec_connection_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
struct nbrec_connection *nbrec_connection_index_init_row(struct ovsdb_idl_index *);
...
const struct nbrec_dhcp_options_table *nbrec_dhcp_options_table_get(const struct ovsdb_idl *);
const struct nbrec_dhcp_options *nbrec_dhcp_options_table_first(const struct nbrec_dhcp_options_table *);

#define NBREC_DHCP_OPTIONS_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = nbrec_dhcp_options_table_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_dhcp_options_next(ROW))
#define NBREC_DHCP_OPTIONS_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = nbrec_dhcp_options_table_first(TABLE); \
...
const struct nbrec_dhcp_options *nbrec_dhcp_options_table_get_for_uuid(const struct nbrec_dhcp_options_table *, const struct uuid *);
...
const struct nbrec_dhcp_options *nbrec_dhcp_options_table_track_get_first(const struct nbrec_dhcp_options_table *);
#define NBREC_DHCP_OPTIONS_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = nbrec_dhcp_options_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_dhcp_options_track_get_next(ROW))
...
struct nbrec_dhcp_options *nbrec_dhcp_options_index_find(struct ovsdb_idl_index *, const struct nbrec_dhcp_options *);

int nbrec_dhcp_options_index_compare(
    struct ovsdb_idl_index *, 
    const struct nbrec_dhcp_options *, 
    const struct nbrec_dhcp_options *);
struct ovsdb_idl_cursor nbrec_dhcp_options_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor nbrec_dhcp_options_cursor_first_eq(
    struct ovsdb_idl_index *, const struct nbrec_dhcp_options *);
struct ovsdb_idl_cursor nbrec_dhcp_options_cursor_first_ge(
    struct ovsdb_idl_index *, const struct nbrec_dhcp_options *);

struct nbrec_dhcp_options *nbrec_dhcp_options_cursor_data(struct ovsdb_idl_cursor *);

#define NBREC_DHCP_OPTIONS_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_dhcp_options_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = nbrec_dhcp_options_cursor_data(&cursor__), \
                  !(TO) || nbrec_dhcp_options_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define NBREC_DHCP_OPTIONS_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_dhcp_options_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = nbrec_dhcp_options_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define NBREC_DHCP_OPTIONS_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_dhcp_options_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = nbrec_dhcp_options_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
struct nbrec_dhcp_options *nbrec_dhcp_options_index_init_row(struct ovsdb_idl_index *);
...
const struct nbrec_dns_table *nbrec_dns_table_get(const struct ovsdb_idl *);
const struct nbrec_dns *nbrec_dns_table_first(const struct nbrec_dns_table *);

#define NBREC_DNS_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = nbrec_dns_table_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_dns_next(ROW))
#define NBREC_DNS_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = nbrec_dns_table_first(TABLE); \
             (ROW) ? ((NEXT) = nbrec_dns_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

const struct nbrec_dns *nbrec_dns_get_for_uuid(const struct ovsdb_idl *, const struct uuid *);
const struct nbrec_dns *nbrec_dns_table_get_for_uuid(const struct nbrec_dns_table *, const struct uuid *);
...
const struct nbrec_dns *nbrec_dns_table_track_get_first(const struct nbrec_dns_table *);
#define NBREC_DNS_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = nbrec_dns_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_dns_track_get_next(ROW))
...
struct nbrec_dns *nbrec_dns_index_find(struct ovsdb_idl_index *, const struct nbrec_dns *);

int nbrec_dns_index_compare(
    struct ovsdb_idl_index *, 
    const struct nbrec_dns *, 
    const struct nbrec_dns *);
struct ovsdb_idl_cursor nbrec_dns_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor nbrec_dns_cursor_first_eq(
    struct ovsdb_idl_index *, const struct nbrec_dns *);
struct ovsdb_idl_cursor nbrec_dns_cursor_first_ge(
    struct ovsdb_idl_index *, const struct nbrec_dns *);

struct nbrec_dns *nbrec_dns_cursor_data(struct ovsdb_idl_cursor *);

#define NBREC_DNS_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_dns_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = nbrec_dns_cursor_data(&cursor__), \
                  !(TO) || nbrec_dns_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define NBREC_DNS_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_dns_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = nbrec_dns_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define NBREC_DNS_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_dns_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = nbrec_dns_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
struct nbrec_dns *nbrec_dns_index_init_row(struct ovsdb_idl_index *);
...
const struct nbrec_gateway_chassis_table *nbrec_gateway_chassis_table_get(const struct ovsdb_idl *);
const struct nbrec_gateway_chassis *nbrec_gateway_chassis_table_first(const struct nbrec_gateway_chassis_table *);

#define NBREC_GATEWAY_CHASSIS_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = nbrec_gateway_chassis_table_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_gateway_chassis_next(ROW))
#define NBREC_GATEWAY_CHASSIS_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = nbrec_gateway_chassis_table_first(TABLE); \
...
const struct nbrec_gateway_chassis *nbrec_gateway_chassis_table_get_for_uuid(const struct nbrec_gateway_chassis_table *, const struct uuid *);
...
const struct nbrec_gateway_chassis *nbrec_gateway_chassis_table_track_get_first(const struct nbrec_gateway_chassis_table *);
#define NBREC_GATEWAY_CHASSIS_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = nbrec_gateway_chassis_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_gateway_chassis_track_get_next(ROW))
...
struct nbrec_gateway_chassis *nbrec_gateway_chassis_index_find(struct ovsdb_idl_index *, const struct nbrec_gateway_chassis *);

int nbrec_gateway_chassis_index_compare(
    struct ovsdb_idl_index *, 
    const struct nbrec_gateway_chassis *, 
    const struct nbrec_gateway_chassis *);
struct ovsdb_idl_cursor nbrec_gateway_chassis_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor nbrec_gateway_chassis_cursor_first_eq(
    struct ovsdb_idl_index *, const struct nbrec_gateway_chassis *);
struct ovsdb_idl_cursor nbrec_gateway_chassis_cursor_first_ge(
    struct ovsdb_idl_index *, const struct nbrec_gateway_chassis *);

struct nbrec_gateway_chassis *nbrec_gateway_chassis_cursor_data(struct ovsdb_idl_cursor *);

#define NBREC_GATEWAY_CHASSIS_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_gateway_chassis_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = nbrec_gateway_chassis_cursor_data(&cursor__), \
                  !(TO) || nbrec_gateway_chassis_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define NBREC_GATEWAY_CHASSIS_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_gateway_chassis_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = nbrec_gateway_chassis_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define NBREC_GATEWAY_CHASSIS_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_gateway_chassis_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = nbrec_gateway_chassis_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
struct nbrec_gateway_chassis *nbrec_gateway_chassis_index_init_row(struct ovsdb_idl_index *);
...
const struct nbrec_load_balancer_table *nbrec_load_balancer_table_get(const struct ovsdb_idl *);
const struct nbrec_load_balancer *nbrec_load_balancer_table_first(const struct nbrec_load_balancer_table *);

#define NBREC_LOAD_BALANCER_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = nbrec_load_balancer_table_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_load_balancer_next(ROW))
#define NBREC_LOAD_BALANCER_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = nbrec_load_balancer_table_first(TABLE); \
...
const struct nbrec_load_balancer *nbrec_load_balancer_table_get_for_uuid(const struct nbrec_load_balancer_table *, const struct uuid *);
...
const struct nbrec_load_balancer *nbrec_load_balancer_table_track_get_first(const struct nbrec_load_balancer_table *);
#define NBREC_LOAD_BALANCER_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = nbrec_load_balancer_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_load_balancer_track_get_next(ROW))
...
struct nbrec_load_balancer *nbrec_load_balancer_index_find(struct ovsdb_idl_index *, const struct nbrec_load_balancer *);

int nbrec_load_balancer_index_compare(
    struct ovsdb_idl_index *, 
    const struct nbrec_load_balancer *, 
    const struct nbrec_load_balancer *);
struct ovsdb_idl_cursor nbrec_load_balancer_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor nbrec_load_balancer_cursor_first_eq(
    struct ovsdb_idl_index *, const struct nbrec_load_balancer *);
struct ovsdb_idl_cursor nbrec_load_balancer_cursor_first_ge(
    struct ovsdb_idl_index *, const struct nbrec_load_balancer *);

struct nbrec_load_balancer *nbrec_load_balancer_cursor_data(struct ovsdb_idl_cursor *);

#define NBREC_LOAD_BALANCER_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_load_balancer_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = nbrec_load_balancer_cursor_data(&cursor__), \
                  !(TO) || nbrec_load_balancer_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define NBREC_LOAD_BALANCER_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_load_balancer_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = nbrec_load_balancer_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define NBREC_LOAD_BALANCER_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_load_balancer_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = nbrec_load_balancer_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
struct nbrec_load_balancer *nbrec_load_balancer_index_init_row(struct ovsdb_idl_index *);
...
const struct nbrec_logical_router_table *nbrec_logical_router_table_get(const struct ovsdb_idl *);
const struct nbrec_logical_router *nbrec_logical_router_table_first(const struct nbrec_logical_router_table *);

#define NBREC_LOGICAL_ROUTER_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = nbrec_logical_router_table_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_logical_router_next(ROW))
#define NBREC_LOGICAL_ROUTER_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = nbrec_logical_router_table_first(TABLE); \
...
const struct nbrec_logical_router *nbrec_logical_router_table_get_for_uuid(const struct nbrec_logical_router_table *, const struct uuid *);
...
const struct nbrec_logical_router *nbrec_logical_router_table_track_get_first(const struct nbrec_logical_router_table *);
#define NBREC_LOGICAL_ROUTER_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = nbrec_logical_router_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_logical_router_track_get_next(ROW))
...
struct nbrec_logical_router *nbrec_logical_router_index_find(struct ovsdb_idl_index *, const struct nbrec_logical_router *);

int nbrec_logical_router_index_compare(
    struct ovsdb_idl_index *, 
    const struct nbrec_logical_router *, 
    const struct nbrec_logical_router *);
struct ovsdb_idl_cursor nbrec_logical_router_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor nbrec_logical_router_cursor_first_eq(
    struct ovsdb_idl_index *, const struct nbrec_logical_router *);
struct ovsdb_idl_cursor nbrec_logical_router_cursor_first_ge(
    struct ovsdb_idl_index *, const struct nbrec_logical_router *);

struct nbrec_logical_router *nbrec_logical_router_cursor_data(struct ovsdb_idl_cursor *);

#define NBREC_LOGICAL_ROUTER_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_logical_router_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = nbrec_logical_router_cursor_data(&cursor__), \
                  !(TO) || nbrec_logical_router_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define NBREC_LOGICAL_ROUTER_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_logical_router_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = nbrec_logical_router_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define NBREC_LOGICAL_ROUTER_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_logical_router_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = nbrec_logical_router_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
struct nbrec_logical_router *nbrec_logical_router_index_init_row(struct ovsdb_idl_index *);
...
struct nbrec_logical_router_port {
	...
	/* ipv6_ra_configs column. */
	struct smap ipv6_ra_configs;
...
#define nbrec_logical_router_port_col_ipv6_ra_configs (nbrec_logical_router_port_columns[NBREC_LOGICAL_ROUTER_PORT_COL_IPV6_RA_CONFIGS])
...
const struct nbrec_logical_router_port_table *nbrec_logical_router_port_table_get(const struct ovsdb_idl *);
const struct nbrec_logical_router_port *nbrec_logical_router_port_table_first(const struct nbrec_logical_router_port_table *);

#define NBREC_LOGICAL_ROUTER_PORT_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = nbrec_logical_router_port_table_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_logical_router_port_next(ROW))
#define NBREC_LOGICAL_ROUTER_PORT_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = nbrec_logical_router_port_table_first(TABLE); \
...
const struct nbrec_logical_router_port *nbrec_logical_router_port_table_get_for_uuid(const struct nbrec_logical_router_port_table *, const struct uuid *);
...
const struct nbrec_logical_router_port *nbrec_logical_router_port_table_track_get_first(const struct nbrec_logical_router_port_table *);
#define NBREC_LOGICAL_ROUTER_PORT_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = nbrec_logical_router_port_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_logical_router_port_track_get_next(ROW))
...
struct nbrec_logical_router_port *nbrec_logical_router_port_index_find(struct ovsdb_idl_index *, const struct nbrec_logical_router_port *);

int nbrec_logical_router_port_index_compare(
    struct ovsdb_idl_index *, 
    const struct nbrec_logical_router_port *, 
    const struct nbrec_logical_router_port *);
struct ovsdb_idl_cursor nbrec_logical_router_port_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor nbrec_logical_router_port_cursor_first_eq(
    struct ovsdb_idl_index *, const struct nbrec_logical_router_port *);
struct ovsdb_idl_cursor nbrec_logical_router_port_cursor_first_ge(
    struct ovsdb_idl_index *, const struct nbrec_logical_router_port *);

struct nbrec_logical_router_port *nbrec_logical_router_port_cursor_data(struct ovsdb_idl_cursor *);

#define NBREC_LOGICAL_ROUTER_PORT_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_logical_router_port_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = nbrec_logical_router_port_cursor_data(&cursor__), \
                  !(TO) || nbrec_logical_router_port_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define NBREC_LOGICAL_ROUTER_PORT_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_logical_router_port_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = nbrec_logical_router_port_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define NBREC_LOGICAL_ROUTER_PORT_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_logical_router_port_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = nbrec_logical_router_port_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
void nbrec_logical_router_port_verify_ipv6_ra_configs(const struct nbrec_logical_router_port *);
...
const struct ovsdb_datum *nbrec_logical_router_port_get_ipv6_ra_configs(const struct nbrec_logical_router_port *, enum ovsdb_atomic_type key_type, enum ovsdb_atomic_type value_type);
...
void nbrec_logical_router_port_set_ipv6_ra_configs(const struct nbrec_logical_router_port *, const struct smap *);
...
void nbrec_logical_router_port_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_logical_router_port_update_ipv6_ra_configs_setkey(const struct nbrec_logical_router_port *,  const char *, const char *);
void nbrec_logical_router_port_update_ipv6_ra_configs_delkey(const struct nbrec_logical_router_port *,  const char *);
void nbrec_logical_router_port_add_clause_ipv6_ra_configs(struct ovsdb_idl_condition *, enum ovsdb_function function, const struct smap *);
void nbrec_logical_router_port_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
...
struct nbrec_logical_router_port *nbrec_logical_router_port_index_init_row(struct ovsdb_idl_index *);
...
void nbrec_logical_router_port_index_set_ipv6_ra_configs(const struct nbrec_logical_router_port *,
const struct smap *);
...
#define nbrec_logical_router_static_route_col_external_ids (nbrec_logical_router_static_route_columns[NBREC_LOGICAL_ROUTER_STATIC_ROUTE_COL_EXTERNAL_IDS])
...
const struct nbrec_logical_router_static_route_table *nbrec_logical_router_static_route_table_get(const struct ovsdb_idl *);
const struct nbrec_logical_router_static_route *nbrec_logical_router_static_route_table_first(const struct nbrec_logical_router_static_route_table *);

#define NBREC_LOGICAL_ROUTER_STATIC_ROUTE_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = nbrec_logical_router_static_route_table_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_logical_router_static_route_next(ROW))
#define NBREC_LOGICAL_ROUTER_STATIC_ROUTE_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = nbrec_logical_router_static_route_table_first(TABLE); \
...
const struct nbrec_logical_router_static_route *nbrec_logical_router_static_route_table_get_for_uuid(const struct nbrec_logical_router_static_route_table *, const struct uuid *);
...
const struct nbrec_logical_router_static_route *nbrec_logical_router_static_route_table_track_get_first(const struct nbrec_logical_router_static_route_table *);
#define NBREC_LOGICAL_ROUTER_STATIC_ROUTE_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = nbrec_logical_router_static_route_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_logical_router_static_route_track_get_next(ROW))
...
struct nbrec_logical_router_static_route *nbrec_logical_router_static_route_index_find(struct ovsdb_idl_index *, const struct nbrec_logical_router_static_route *);

int nbrec_logical_router_static_route_index_compare(
    struct ovsdb_idl_index *, 
    const struct nbrec_logical_router_static_route *, 
    const struct nbrec_logical_router_static_route *);
struct ovsdb_idl_cursor nbrec_logical_router_static_route_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor nbrec_logical_router_static_route_cursor_first_eq(
    struct ovsdb_idl_index *, const struct nbrec_logical_router_static_route *);
struct ovsdb_idl_cursor nbrec_logical_router_static_route_cursor_first_ge(
    struct ovsdb_idl_index *, const struct nbrec_logical_router_static_route *);

struct nbrec_logical_router_static_route *nbrec_logical_router_static_route_cursor_data(struct ovsdb_idl_cursor *);

#define NBREC_LOGICAL_ROUTER_STATIC_ROUTE_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_logical_router_static_route_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = nbrec_logical_router_static_route_cursor_data(&cursor__), \
                  !(TO) || nbrec_logical_router_static_route_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define NBREC_LOGICAL_ROUTER_STATIC_ROUTE_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_logical_router_static_route_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = nbrec_logical_router_static_route_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define NBREC_LOGICAL_ROUTER_STATIC_ROUTE_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_logical_router_static_route_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = nbrec_logical_router_static_route_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
void nbrec_logical_router_static_route_verify_external_ids(const struct nbrec_logical_router_static_route *);
...
const struct ovsdb_datum *nbrec_logical_router_static_route_get_external_ids(const struct nbrec_logical_router_static_route *, enum ovsdb_atomic_type key_type, enum ovsdb_atomic_type value_type);
...
void nbrec_logical_router_static_route_set_external_ids(const struct nbrec_logical_router_static_route *, const struct smap *);
...
void nbrec_logical_router_static_route_update_external_ids_setkey(const struct nbrec_logical_router_static_route *,  const char *, const char *);
void nbrec_logical_router_static_route_update_external_ids_delkey(const struct nbrec_logical_router_static_route *,  const char *);
void nbrec_logical_router_static_route_add_clause_external_ids(struct ovsdb_idl_condition *, enum ovsdb_function function, const struct smap *);
...
struct nbrec_logical_router_static_route *nbrec_logical_router_static_route_index_init_row(struct ovsdb_idl_index *);
void nbrec_logical_router_static_route_index_set_external_ids(const struct nbrec_logical_router_static_route *,
const struct smap *);
...
const struct nbrec_logical_switch_table *nbrec_logical_switch_table_get(const struct ovsdb_idl *);
const struct nbrec_logical_switch *nbrec_logical_switch_table_first(const struct nbrec_logical_switch_table *);

#define NBREC_LOGICAL_SWITCH_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = nbrec_logical_switch_table_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_logical_switch_next(ROW))
#define NBREC_LOGICAL_SWITCH_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = nbrec_logical_switch_table_first(TABLE); \
...
const struct nbrec_logical_switch *nbrec_logical_switch_table_get_for_uuid(const struct nbrec_logical_switch_table *, const struct uuid *);
...
const struct nbrec_logical_switch *nbrec_logical_switch_table_track_get_first(const struct nbrec_logical_switch_table *);
#define NBREC_LOGICAL_SWITCH_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = nbrec_logical_switch_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_logical_switch_track_get_next(ROW))
...
struct nbrec_logical_switch *nbrec_logical_switch_index_find(struct ovsdb_idl_index *, const struct nbrec_logical_switch *);

int nbrec_logical_switch_index_compare(
    struct ovsdb_idl_index *, 
    const struct nbrec_logical_switch *, 
    const struct nbrec_logical_switch *);
struct ovsdb_idl_cursor nbrec_logical_switch_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor nbrec_logical_switch_cursor_first_eq(
    struct ovsdb_idl_index *, const struct nbrec_logical_switch *);
struct ovsdb_idl_cursor nbrec_logical_switch_cursor_first_ge(
    struct ovsdb_idl_index *, const struct nbrec_logical_switch *);

struct nbrec_logical_switch *nbrec_logical_switch_cursor_data(struct ovsdb_idl_cursor *);

#define NBREC_LOGICAL_SWITCH_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_logical_switch_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = nbrec_logical_switch_cursor_data(&cursor__), \
                  !(TO) || nbrec_logical_switch_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define NBREC_LOGICAL_SWITCH_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_logical_switch_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = nbrec_logical_switch_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define NBREC_LOGICAL_SWITCH_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_logical_switch_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = nbrec_logical_switch_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
struct nbrec_logical_switch *nbrec_logical_switch_index_init_row(struct ovsdb_idl_index *);
...
const struct nbrec_logical_switch_port_table *nbrec_logical_switch_port_table_get(const struct ovsdb_idl *);
const struct nbrec_logical_switch_port *nbrec_logical_switch_port_table_first(const struct nbrec_logical_switch_port_table *);

#define NBREC_LOGICAL_SWITCH_PORT_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = nbrec_logical_switch_port_table_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_logical_switch_port_next(ROW))
#define NBREC_LOGICAL_SWITCH_PORT_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = nbrec_logical_switch_port_table_first(TABLE); \
...
const struct nbrec_logical_switch_port *nbrec_logical_switch_port_table_get_for_uuid(const struct nbrec_logical_switch_port_table *, const struct uuid *);
...
const struct nbrec_logical_switch_port *nbrec_logical_switch_port_table_track_get_first(const struct nbrec_logical_switch_port_table *);
#define NBREC_LOGICAL_SWITCH_PORT_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = nbrec_logical_switch_port_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_logical_switch_port_track_get_next(ROW))
...
struct nbrec_logical_switch_port *nbrec_logical_switch_port_index_find(struct ovsdb_idl_index *, const struct nbrec_logical_switch_port *);

int nbrec_logical_switch_port_index_compare(
    struct ovsdb_idl_index *, 
    const struct nbrec_logical_switch_port *, 
    const struct nbrec_logical_switch_port *);
struct ovsdb_idl_cursor nbrec_logical_switch_port_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor nbrec_logical_switch_port_cursor_first_eq(
    struct ovsdb_idl_index *, const struct nbrec_logical_switch_port *);
struct ovsdb_idl_cursor nbrec_logical_switch_port_cursor_first_ge(
    struct ovsdb_idl_index *, const struct nbrec_logical_switch_port *);

struct nbrec_logical_switch_port *nbrec_logical_switch_port_cursor_data(struct ovsdb_idl_cursor *);

#define NBREC_LOGICAL_SWITCH_PORT_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_logical_switch_port_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = nbrec_logical_switch_port_cursor_data(&cursor__), \
                  !(TO) || nbrec_logical_switch_port_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define NBREC_LOGICAL_SWITCH_PORT_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_logical_switch_port_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = nbrec_logical_switch_port_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define NBREC_LOGICAL_SWITCH_PORT_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_logical_switch_port_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = nbrec_logical_switch_port_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
struct nbrec_logical_switch_port *nbrec_logical_switch_port_index_init_row(struct ovsdb_idl_index *);
...
enum nbrec_meter_column_id {
    NBREC_METER_COL_BANDS,
    NBREC_METER_COL_EXTERNAL_IDS,
    NBREC_METER_COL_NAME,
    NBREC_METER_COL_UNIT,
    NBREC_METER_N_COLUMNS
};

#define nbrec_meter_col_bands (nbrec_meter_columns[NBREC_METER_COL_BANDS])
#define nbrec_meter_col_external_ids (nbrec_meter_columns[NBREC_METER_COL_EXTERNAL_IDS])
#define nbrec_meter_col_name (nbrec_meter_columns[NBREC_METER_COL_NAME])
#define nbrec_meter_col_unit (nbrec_meter_columns[NBREC_METER_COL_UNIT])

extern struct ovsdb_idl_column nbrec_meter_columns[NBREC_METER_N_COLUMNS];

const struct nbrec_meter_table *nbrec_meter_table_get(const struct ovsdb_idl *);
const struct nbrec_meter *nbrec_meter_table_first(const struct nbrec_meter_table *);

#define NBREC_METER_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = nbrec_meter_table_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_meter_next(ROW))
#define NBREC_METER_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = nbrec_meter_table_first(TABLE); \
             (ROW) ? ((NEXT) = nbrec_meter_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

const struct nbrec_meter *nbrec_meter_get_for_uuid(const struct ovsdb_idl *, const struct uuid *);
const struct nbrec_meter *nbrec_meter_table_get_for_uuid(const struct nbrec_meter_table *, const struct uuid *);
const struct nbrec_meter *nbrec_meter_first(const struct ovsdb_idl *);
const struct nbrec_meter *nbrec_meter_next(const struct nbrec_meter *);
#define NBREC_METER_FOR_EACH(ROW, IDL) \
        for ((ROW) = nbrec_meter_first(IDL); \
             (ROW); \
             (ROW) = nbrec_meter_next(ROW))
#define NBREC_METER_FOR_EACH_SAFE(ROW, NEXT, IDL) \
        for ((ROW) = nbrec_meter_first(IDL); \
             (ROW) ? ((NEXT) = nbrec_meter_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

unsigned int nbrec_meter_get_seqno(const struct ovsdb_idl *);
unsigned int nbrec_meter_row_get_seqno(const struct nbrec_meter *row, enum ovsdb_idl_change change);
const struct nbrec_meter *nbrec_meter_track_get_first(const struct ovsdb_idl *);
const struct nbrec_meter *nbrec_meter_track_get_next(const struct nbrec_meter *);
#define NBREC_METER_FOR_EACH_TRACKED(ROW, IDL) \
        for ((ROW) = nbrec_meter_track_get_first(IDL); \
             (ROW); \
             (ROW) = nbrec_meter_track_get_next(ROW))

const struct nbrec_meter *nbrec_meter_table_track_get_first(const struct nbrec_meter_table *);
#define NBREC_METER_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = nbrec_meter_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_meter_track_get_next(ROW))


/* Returns true if 'row' was inserted since the last change tracking reset. */
static inline bool nbrec_meter_is_new(const struct nbrec_meter *row)
{
    return nbrec_meter_row_get_seqno(row, OVSDB_IDL_CHANGE_MODIFY) == 0;
}

/* Returns true if 'row' was deleted since the last change tracking reset. */
static inline bool nbrec_meter_is_deleted(const struct nbrec_meter *row)
{
    return nbrec_meter_row_get_seqno(row, OVSDB_IDL_CHANGE_DELETE) > 0;
}

void nbrec_meter_index_destroy_row(const struct nbrec_meter *);

struct nbrec_meter *nbrec_meter_index_find(struct ovsdb_idl_index *, const struct nbrec_meter *);

int nbrec_meter_index_compare(
    struct ovsdb_idl_index *, 
    const struct nbrec_meter *, 
    const struct nbrec_meter *);
struct ovsdb_idl_cursor nbrec_meter_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor nbrec_meter_cursor_first_eq(
    struct ovsdb_idl_index *, const struct nbrec_meter *);
struct ovsdb_idl_cursor nbrec_meter_cursor_first_ge(
    struct ovsdb_idl_index *, const struct nbrec_meter *);

struct nbrec_meter *nbrec_meter_cursor_data(struct ovsdb_idl_cursor *);

#define NBREC_METER_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_meter_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = nbrec_meter_cursor_data(&cursor__), \
                  !(TO) || nbrec_meter_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define NBREC_METER_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_meter_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = nbrec_meter_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define NBREC_METER_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_meter_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = nbrec_meter_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
void nbrec_meter_init(struct nbrec_meter *);
void nbrec_meter_delete(const struct nbrec_meter *);
struct nbrec_meter *nbrec_meter_insert(struct ovsdb_idl_txn *);
bool nbrec_meter_is_updated(const struct nbrec_meter *, enum nbrec_meter_column_id);

void nbrec_meter_verify_bands(const struct nbrec_meter *);
void nbrec_meter_verify_external_ids(const struct nbrec_meter *);
void nbrec_meter_verify_name(const struct nbrec_meter *);
void nbrec_meter_verify_unit(const struct nbrec_meter *);

const struct ovsdb_datum *nbrec_meter_get_bands(const struct nbrec_meter *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *nbrec_meter_get_external_ids(const struct nbrec_meter *, enum ovsdb_atomic_type key_type, enum ovsdb_atomic_type value_type);
const struct ovsdb_datum *nbrec_meter_get_name(const struct nbrec_meter *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *nbrec_meter_get_unit(const struct nbrec_meter *, enum ovsdb_atomic_type key_type);

void nbrec_meter_set_bands(const struct nbrec_meter *, struct nbrec_meter_band **bands, size_t n_bands);
void nbrec_meter_set_external_ids(const struct nbrec_meter *, const struct smap *);
void nbrec_meter_set_name(const struct nbrec_meter *, const char *name);
void nbrec_meter_set_unit(const struct nbrec_meter *, const char *unit);

void nbrec_meter_update_bands_addvalue(const struct nbrec_meter *,  const struct nbrec_meter_band *);
void nbrec_meter_update_bands_delvalue(const struct nbrec_meter *,  const struct nbrec_meter_band *);
void nbrec_meter_add_clause_bands(struct ovsdb_idl_condition *, enum ovsdb_function function, struct uuid **bands, size_t n_bands);
void nbrec_meter_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_meter_update_external_ids_setkey(const struct nbrec_meter *,  const char *, const char *);
void nbrec_meter_update_external_ids_delkey(const struct nbrec_meter *,  const char *);
void nbrec_meter_add_clause_external_ids(struct ovsdb_idl_condition *, enum ovsdb_function function, const struct smap *);
void nbrec_meter_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_meter_add_clause_name(struct ovsdb_idl_condition *, enum ovsdb_function function, const char *name);
void nbrec_meter_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_meter_add_clause_unit(struct ovsdb_idl_condition *, enum ovsdb_function function, const char *unit);
void nbrec_meter_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);

struct nbrec_meter *nbrec_meter_index_init_row(struct ovsdb_idl_index *);
void nbrec_meter_index_set_bands(const struct nbrec_meter *,
struct nbrec_meter_band **bands, size_t n_bands);
void nbrec_meter_index_set_external_ids(const struct nbrec_meter *,
const struct smap *);
void nbrec_meter_index_set_name(const struct nbrec_meter *,
const char *name);
void nbrec_meter_index_set_unit(const struct nbrec_meter *,
const char *unit);

/* Meter_Band table. */
...
enum nbrec_meter_band_column_id {
    NBREC_METER_BAND_COL_ACTION,
    NBREC_METER_BAND_COL_BURST_SIZE,
    NBREC_METER_BAND_COL_EXTERNAL_IDS,
    NBREC_METER_BAND_COL_RATE,
    NBREC_METER_BAND_N_COLUMNS
};

#define nbrec_meter_band_col_action (nbrec_meter_band_columns[NBREC_METER_BAND_COL_ACTION])
#define nbrec_meter_band_col_external_ids (nbrec_meter_band_columns[NBREC_METER_BAND_COL_EXTERNAL_IDS])
#define nbrec_meter_band_col_burst_size (nbrec_meter_band_columns[NBREC_METER_BAND_COL_BURST_SIZE])
#define nbrec_meter_band_col_rate (nbrec_meter_band_columns[NBREC_METER_BAND_COL_RATE])

extern struct ovsdb_idl_column nbrec_meter_band_columns[NBREC_METER_BAND_N_COLUMNS];

const struct nbrec_meter_band_table *nbrec_meter_band_table_get(const struct ovsdb_idl *);
const struct nbrec_meter_band *nbrec_meter_band_table_first(const struct nbrec_meter_band_table *);

#define NBREC_METER_BAND_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = nbrec_meter_band_table_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_meter_band_next(ROW))
#define NBREC_METER_BAND_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = nbrec_meter_band_table_first(TABLE); \
             (ROW) ? ((NEXT) = nbrec_meter_band_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

const struct nbrec_meter_band *nbrec_meter_band_get_for_uuid(const struct ovsdb_idl *, const struct uuid *);
const struct nbrec_meter_band *nbrec_meter_band_table_get_for_uuid(const struct nbrec_meter_band_table *, const struct uuid *);
const struct nbrec_meter_band *nbrec_meter_band_first(const struct ovsdb_idl *);
const struct nbrec_meter_band *nbrec_meter_band_next(const struct nbrec_meter_band *);
#define NBREC_METER_BAND_FOR_EACH(ROW, IDL) \
        for ((ROW) = nbrec_meter_band_first(IDL); \
             (ROW); \
             (ROW) = nbrec_meter_band_next(ROW))
#define NBREC_METER_BAND_FOR_EACH_SAFE(ROW, NEXT, IDL) \
        for ((ROW) = nbrec_meter_band_first(IDL); \
             (ROW) ? ((NEXT) = nbrec_meter_band_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

unsigned int nbrec_meter_band_get_seqno(const struct ovsdb_idl *);
unsigned int nbrec_meter_band_row_get_seqno(const struct nbrec_meter_band *row, enum ovsdb_idl_change change);
const struct nbrec_meter_band *nbrec_meter_band_track_get_first(const struct ovsdb_idl *);
const struct nbrec_meter_band *nbrec_meter_band_track_get_next(const struct nbrec_meter_band *);
#define NBREC_METER_BAND_FOR_EACH_TRACKED(ROW, IDL) \
        for ((ROW) = nbrec_meter_band_track_get_first(IDL); \
             (ROW); \
             (ROW) = nbrec_meter_band_track_get_next(ROW))

const struct nbrec_meter_band *nbrec_meter_band_table_track_get_first(const struct nbrec_meter_band_table *);
#define NBREC_METER_BAND_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = nbrec_meter_band_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_meter_band_track_get_next(ROW))


/* Returns true if 'row' was inserted since the last change tracking reset. */
static inline bool nbrec_meter_band_is_new(const struct nbrec_meter_band *row)
{
    return nbrec_meter_band_row_get_seqno(row, OVSDB_IDL_CHANGE_MODIFY) == 0;
}

/* Returns true if 'row' was deleted since the last change tracking reset. */
static inline bool nbrec_meter_band_is_deleted(const struct nbrec_meter_band *row)
{
    return nbrec_meter_band_row_get_seqno(row, OVSDB_IDL_CHANGE_DELETE) > 0;
}

void nbrec_meter_band_index_destroy_row(const struct nbrec_meter_band *);

struct nbrec_meter_band *nbrec_meter_band_index_find(struct ovsdb_idl_index *, const struct nbrec_meter_band *);

int nbrec_meter_band_index_compare(
    struct ovsdb_idl_index *, 
    const struct nbrec_meter_band *, 
    const struct nbrec_meter_band *);
struct ovsdb_idl_cursor nbrec_meter_band_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor nbrec_meter_band_cursor_first_eq(
    struct ovsdb_idl_index *, const struct nbrec_meter_band *);
struct ovsdb_idl_cursor nbrec_meter_band_cursor_first_ge(
    struct ovsdb_idl_index *, const struct nbrec_meter_band *);

struct nbrec_meter_band *nbrec_meter_band_cursor_data(struct ovsdb_idl_cursor *);

#define NBREC_METER_BAND_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_meter_band_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = nbrec_meter_band_cursor_data(&cursor__), \
                  !(TO) || nbrec_meter_band_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define NBREC_METER_BAND_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_meter_band_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = nbrec_meter_band_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define NBREC_METER_BAND_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_meter_band_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = nbrec_meter_band_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
void nbrec_meter_band_init(struct nbrec_meter_band *);
void nbrec_meter_band_delete(const struct nbrec_meter_band *);
struct nbrec_meter_band *nbrec_meter_band_insert(struct ovsdb_idl_txn *);
bool nbrec_meter_band_is_updated(const struct nbrec_meter_band *, enum nbrec_meter_band_column_id);

void nbrec_meter_band_verify_action(const struct nbrec_meter_band *);
void nbrec_meter_band_verify_burst_size(const struct nbrec_meter_band *);
void nbrec_meter_band_verify_external_ids(const struct nbrec_meter_band *);
void nbrec_meter_band_verify_rate(const struct nbrec_meter_band *);

const struct ovsdb_datum *nbrec_meter_band_get_action(const struct nbrec_meter_band *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *nbrec_meter_band_get_burst_size(const struct nbrec_meter_band *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *nbrec_meter_band_get_external_ids(const struct nbrec_meter_band *, enum ovsdb_atomic_type key_type, enum ovsdb_atomic_type value_type);
const struct ovsdb_datum *nbrec_meter_band_get_rate(const struct nbrec_meter_band *, enum ovsdb_atomic_type key_type);

void nbrec_meter_band_set_action(const struct nbrec_meter_band *, const char *action);
void nbrec_meter_band_set_burst_size(const struct nbrec_meter_band *, int64_t burst_size);
void nbrec_meter_band_set_external_ids(const struct nbrec_meter_band *, const struct smap *);
void nbrec_meter_band_set_rate(const struct nbrec_meter_band *, int64_t rate);

void nbrec_meter_band_add_clause_action(struct ovsdb_idl_condition *, enum ovsdb_function function, const char *action);
void nbrec_meter_band_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_meter_band_add_clause_burst_size(struct ovsdb_idl_condition *, enum ovsdb_function function, int64_t burst_size);
void nbrec_meter_band_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_meter_band_update_external_ids_setkey(const struct nbrec_meter_band *,  const char *, const char *);
void nbrec_meter_band_update_external_ids_delkey(const struct nbrec_meter_band *,  const char *);
void nbrec_meter_band_add_clause_external_ids(struct ovsdb_idl_condition *, enum ovsdb_function function, const struct smap *);
void nbrec_meter_band_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_meter_band_add_clause_rate(struct ovsdb_idl_condition *, enum ovsdb_function function, int64_t rate);
void nbrec_meter_band_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);

struct nbrec_meter_band *nbrec_meter_band_index_init_row(struct ovsdb_idl_index *);
void nbrec_meter_band_index_set_action(const struct nbrec_meter_band *,
const char *action);
void nbrec_meter_band_index_set_burst_size(const struct nbrec_meter_band *,
int64_t burst_size);
void nbrec_meter_band_index_set_external_ids(const struct nbrec_meter_band *,
const struct smap *);
void nbrec_meter_band_index_set_rate(const struct nbrec_meter_band *,
int64_t rate);

/* NAT table. */
struct nbrec_nat {
...
#define nbrec_nat_col_external_ids (nbrec_nat_columns[NBREC_NAT_COL_EXTERNAL_IDS])
...
const struct nbrec_nat_table *nbrec_nat_table_get(const struct ovsdb_idl *);
const struct nbrec_nat *nbrec_nat_table_first(const struct nbrec_nat_table *);

#define NBREC_NAT_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = nbrec_nat_table_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_nat_next(ROW))
#define NBREC_NAT_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = nbrec_nat_table_first(TABLE); \
             (ROW) ? ((NEXT) = nbrec_nat_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

const struct nbrec_nat *nbrec_nat_get_for_uuid(const struct ovsdb_idl *, const struct uuid *);
const struct nbrec_nat *nbrec_nat_table_get_for_uuid(const struct nbrec_nat_table *, const struct uuid *);
...
const struct nbrec_nat *nbrec_nat_table_track_get_first(const struct nbrec_nat_table *);
#define NBREC_NAT_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = nbrec_nat_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_nat_track_get_next(ROW))
...
struct nbrec_nat *nbrec_nat_index_find(struct ovsdb_idl_index *, const struct nbrec_nat *);

int nbrec_nat_index_compare(
    struct ovsdb_idl_index *, 
    const struct nbrec_nat *, 
    const struct nbrec_nat *);
struct ovsdb_idl_cursor nbrec_nat_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor nbrec_nat_cursor_first_eq(
    struct ovsdb_idl_index *, const struct nbrec_nat *);
struct ovsdb_idl_cursor nbrec_nat_cursor_first_ge(
    struct ovsdb_idl_index *, const struct nbrec_nat *);

struct nbrec_nat *nbrec_nat_cursor_data(struct ovsdb_idl_cursor *);

#define NBREC_NAT_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_nat_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = nbrec_nat_cursor_data(&cursor__), \
                  !(TO) || nbrec_nat_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define NBREC_NAT_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_nat_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = nbrec_nat_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define NBREC_NAT_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_nat_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = nbrec_nat_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
void nbrec_nat_verify_external_ids(const struct nbrec_nat *);
...
const struct ovsdb_datum *nbrec_nat_get_external_ids(const struct nbrec_nat *, enum ovsdb_atomic_type key_type, enum ovsdb_atomic_type value_type);
...
void nbrec_nat_set_external_ids(const struct nbrec_nat *, const struct smap *);
...
void nbrec_nat_update_external_ids_setkey(const struct nbrec_nat *,  const char *, const char *);
void nbrec_nat_update_external_ids_delkey(const struct nbrec_nat *,  const char *);
void nbrec_nat_add_clause_external_ids(struct ovsdb_idl_condition *, enum ovsdb_function function, const struct smap *);
...
struct nbrec_nat *nbrec_nat_index_init_row(struct ovsdb_idl_index *);
void nbrec_nat_index_set_external_ids(const struct nbrec_nat *,
const struct smap *);
...
#define nbrec_nb_global_col_options (nbrec_nb_global_columns[NBREC_NB_GLOBAL_COL_OPTIONS])
#define nbrec_nb_global_col_ipsec (nbrec_nb_global_columns[NBREC_NB_GLOBAL_COL_IPSEC])

extern struct ovsdb_idl_column nbrec_nb_global_columns[NBREC_NB_GLOBAL_N_COLUMNS];

const struct nbrec_nb_global_table *nbrec_nb_global_table_get(const struct ovsdb_idl *);
const struct nbrec_nb_global *nbrec_nb_global_table_first(const struct nbrec_nb_global_table *);

#define NBREC_NB_GLOBAL_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = nbrec_nb_global_table_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_nb_global_next(ROW))
#define NBREC_NB_GLOBAL_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = nbrec_nb_global_table_first(TABLE); \
...
const struct nbrec_nb_global *nbrec_nb_global_table_get_for_uuid(const struct nbrec_nb_global_table *, const struct uuid *);
...
const struct nbrec_nb_global *nbrec_nb_global_table_track_get_first(const struct nbrec_nb_global_table *);
#define NBREC_NB_GLOBAL_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = nbrec_nb_global_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_nb_global_track_get_next(ROW))
...
struct nbrec_nb_global *nbrec_nb_global_index_find(struct ovsdb_idl_index *, const struct nbrec_nb_global *);

int nbrec_nb_global_index_compare(
    struct ovsdb_idl_index *, 
    const struct nbrec_nb_global *, 
    const struct nbrec_nb_global *);
struct ovsdb_idl_cursor nbrec_nb_global_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor nbrec_nb_global_cursor_first_eq(
    struct ovsdb_idl_index *, const struct nbrec_nb_global *);
struct ovsdb_idl_cursor nbrec_nb_global_cursor_first_ge(
    struct ovsdb_idl_index *, const struct nbrec_nb_global *);

struct nbrec_nb_global *nbrec_nb_global_cursor_data(struct ovsdb_idl_cursor *);

#define NBREC_NB_GLOBAL_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_nb_global_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = nbrec_nb_global_cursor_data(&cursor__), \
                  !(TO) || nbrec_nb_global_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define NBREC_NB_GLOBAL_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_nb_global_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = nbrec_nb_global_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define NBREC_NB_GLOBAL_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_nb_global_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = nbrec_nb_global_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
void nbrec_nb_global_verify_ipsec(const struct nbrec_nb_global *);
void nbrec_nb_global_verify_nb_cfg(const struct nbrec_nb_global *);
void nbrec_nb_global_verify_options(const struct nbrec_nb_global *);
...
const struct ovsdb_datum *nbrec_nb_global_get_ipsec(const struct nbrec_nb_global *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *nbrec_nb_global_get_nb_cfg(const struct nbrec_nb_global *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *nbrec_nb_global_get_options(const struct nbrec_nb_global *, enum ovsdb_atomic_type key_type, enum ovsdb_atomic_type value_type);
...
void nbrec_nb_global_set_ipsec(const struct nbrec_nb_global *, bool ipsec);
void nbrec_nb_global_set_nb_cfg(const struct nbrec_nb_global *, int64_t nb_cfg);
void nbrec_nb_global_set_options(const struct nbrec_nb_global *, const struct smap *);
...
void nbrec_nb_global_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_nb_global_add_clause_ipsec(struct ovsdb_idl_condition *, enum ovsdb_function function, bool ipsec);
void nbrec_nb_global_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_nb_global_add_clause_nb_cfg(struct ovsdb_idl_condition *, enum ovsdb_function function, int64_t nb_cfg);
void nbrec_nb_global_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_nb_global_update_options_setkey(const struct nbrec_nb_global *,  const char *, const char *);
void nbrec_nb_global_update_options_delkey(const struct nbrec_nb_global *,  const char *);
void nbrec_nb_global_add_clause_options(struct ovsdb_idl_condition *, enum ovsdb_function function, const struct smap *);
void nbrec_nb_global_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
...
struct nbrec_nb_global *nbrec_nb_global_index_init_row(struct ovsdb_idl_index *);
...
int64_t hv_cfg);
void nbrec_nb_global_index_set_ipsec(const struct nbrec_nb_global *,
bool ipsec);
void nbrec_nb_global_index_set_nb_cfg(const struct nbrec_nb_global *,
int64_t nb_cfg);
void nbrec_nb_global_index_set_options(const struct nbrec_nb_global *,
const struct smap *);
...
enum nbrec_port_group_column_id {
    NBREC_PORT_GROUP_COL_ACLS,
    NBREC_PORT_GROUP_COL_EXTERNAL_IDS,
    NBREC_PORT_GROUP_COL_NAME,
    NBREC_PORT_GROUP_COL_PORTS,
    NBREC_PORT_GROUP_N_COLUMNS
};

#define nbrec_port_group_col_external_ids (nbrec_port_group_columns[NBREC_PORT_GROUP_COL_EXTERNAL_IDS])
#define nbrec_port_group_col_acls (nbrec_port_group_columns[NBREC_PORT_GROUP_COL_ACLS])
#define nbrec_port_group_col_name (nbrec_port_group_columns[NBREC_PORT_GROUP_COL_NAME])
#define nbrec_port_group_col_ports (nbrec_port_group_columns[NBREC_PORT_GROUP_COL_PORTS])

extern struct ovsdb_idl_column nbrec_port_group_columns[NBREC_PORT_GROUP_N_COLUMNS];

const struct nbrec_port_group_table *nbrec_port_group_table_get(const struct ovsdb_idl *);
const struct nbrec_port_group *nbrec_port_group_table_first(const struct nbrec_port_group_table *);

#define NBREC_PORT_GROUP_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = nbrec_port_group_table_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_port_group_next(ROW))
#define NBREC_PORT_GROUP_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = nbrec_port_group_table_first(TABLE); \
             (ROW) ? ((NEXT) = nbrec_port_group_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

const struct nbrec_port_group *nbrec_port_group_get_for_uuid(const struct ovsdb_idl *, const struct uuid *);
const struct nbrec_port_group *nbrec_port_group_table_get_for_uuid(const struct nbrec_port_group_table *, const struct uuid *);
const struct nbrec_port_group *nbrec_port_group_first(const struct ovsdb_idl *);
const struct nbrec_port_group *nbrec_port_group_next(const struct nbrec_port_group *);
#define NBREC_PORT_GROUP_FOR_EACH(ROW, IDL) \
        for ((ROW) = nbrec_port_group_first(IDL); \
             (ROW); \
             (ROW) = nbrec_port_group_next(ROW))
#define NBREC_PORT_GROUP_FOR_EACH_SAFE(ROW, NEXT, IDL) \
        for ((ROW) = nbrec_port_group_first(IDL); \
             (ROW) ? ((NEXT) = nbrec_port_group_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

unsigned int nbrec_port_group_get_seqno(const struct ovsdb_idl *);
unsigned int nbrec_port_group_row_get_seqno(const struct nbrec_port_group *row, enum ovsdb_idl_change change);
const struct nbrec_port_group *nbrec_port_group_track_get_first(const struct ovsdb_idl *);
const struct nbrec_port_group *nbrec_port_group_track_get_next(const struct nbrec_port_group *);
#define NBREC_PORT_GROUP_FOR_EACH_TRACKED(ROW, IDL) \
        for ((ROW) = nbrec_port_group_track_get_first(IDL); \
             (ROW); \
             (ROW) = nbrec_port_group_track_get_next(ROW))

const struct nbrec_port_group *nbrec_port_group_table_track_get_first(const struct nbrec_port_group_table *);
#define NBREC_PORT_GROUP_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = nbrec_port_group_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_port_group_track_get_next(ROW))


/* Returns true if 'row' was inserted since the last change tracking reset. */
static inline bool nbrec_port_group_is_new(const struct nbrec_port_group *row)
{
    return nbrec_port_group_row_get_seqno(row, OVSDB_IDL_CHANGE_MODIFY) == 0;
}

/* Returns true if 'row' was deleted since the last change tracking reset. */
static inline bool nbrec_port_group_is_deleted(const struct nbrec_port_group *row)
{
    return nbrec_port_group_row_get_seqno(row, OVSDB_IDL_CHANGE_DELETE) > 0;
}

void nbrec_port_group_index_destroy_row(const struct nbrec_port_group *);

struct nbrec_port_group *nbrec_port_group_index_find(struct ovsdb_idl_index *, const struct nbrec_port_group *);

int nbrec_port_group_index_compare(
    struct ovsdb_idl_index *, 
    const struct nbrec_port_group *, 
    const struct nbrec_port_group *);
struct ovsdb_idl_cursor nbrec_port_group_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor nbrec_port_group_cursor_first_eq(
    struct ovsdb_idl_index *, const struct nbrec_port_group *);
struct ovsdb_idl_cursor nbrec_port_group_cursor_first_ge(
    struct ovsdb_idl_index *, const struct nbrec_port_group *);

struct nbrec_port_group *nbrec_port_group_cursor_data(struct ovsdb_idl_cursor *);

#define NBREC_PORT_GROUP_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_port_group_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = nbrec_port_group_cursor_data(&cursor__), \
                  !(TO) || nbrec_port_group_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define NBREC_PORT_GROUP_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_port_group_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = nbrec_port_group_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define NBREC_PORT_GROUP_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_port_group_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = nbrec_port_group_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
void nbrec_port_group_init(struct nbrec_port_group *);
void nbrec_port_group_delete(const struct nbrec_port_group *);
struct nbrec_port_group *nbrec_port_group_insert(struct ovsdb_idl_txn *);
bool nbrec_port_group_is_updated(const struct nbrec_port_group *, enum nbrec_port_group_column_id);

void nbrec_port_group_verify_acls(const struct nbrec_port_group *);
void nbrec_port_group_verify_external_ids(const struct nbrec_port_group *);
void nbrec_port_group_verify_name(const struct nbrec_port_group *);
void nbrec_port_group_verify_ports(const struct nbrec_port_group *);

const struct ovsdb_datum *nbrec_port_group_get_acls(const struct nbrec_port_group *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *nbrec_port_group_get_external_ids(const struct nbrec_port_group *, enum ovsdb_atomic_type key_type, enum ovsdb_atomic_type value_type);
const struct ovsdb_datum *nbrec_port_group_get_name(const struct nbrec_port_group *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *nbrec_port_group_get_ports(const struct nbrec_port_group *, enum ovsdb_atomic_type key_type);

void nbrec_port_group_set_acls(const struct nbrec_port_group *, struct nbrec_acl **acls, size_t n_acls);
void nbrec_port_group_set_external_ids(const struct nbrec_port_group *, const struct smap *);
void nbrec_port_group_set_name(const struct nbrec_port_group *, const char *name);
void nbrec_port_group_set_ports(const struct nbrec_port_group *, struct nbrec_logical_switch_port **ports, size_t n_ports);

void nbrec_port_group_update_acls_addvalue(const struct nbrec_port_group *,  const struct nbrec_acl *);
void nbrec_port_group_update_acls_delvalue(const struct nbrec_port_group *,  const struct nbrec_acl *);
void nbrec_port_group_add_clause_acls(struct ovsdb_idl_condition *, enum ovsdb_function function, struct uuid **acls, size_t n_acls);
void nbrec_port_group_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_port_group_update_external_ids_setkey(const struct nbrec_port_group *,  const char *, const char *);
void nbrec_port_group_update_external_ids_delkey(const struct nbrec_port_group *,  const char *);
void nbrec_port_group_add_clause_external_ids(struct ovsdb_idl_condition *, enum ovsdb_function function, const struct smap *);
void nbrec_port_group_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_port_group_add_clause_name(struct ovsdb_idl_condition *, enum ovsdb_function function, const char *name);
void nbrec_port_group_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_port_group_update_ports_addvalue(const struct nbrec_port_group *,  const struct nbrec_logical_switch_port *);
void nbrec_port_group_update_ports_delvalue(const struct nbrec_port_group *,  const struct nbrec_logical_switch_port *);
void nbrec_port_group_add_clause_ports(struct ovsdb_idl_condition *, enum ovsdb_function function, struct uuid **ports, size_t n_ports);
void nbrec_port_group_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);

struct nbrec_port_group *nbrec_port_group_index_init_row(struct ovsdb_idl_index *);
void nbrec_port_group_index_set_acls(const struct nbrec_port_group *,
struct nbrec_acl **acls, size_t n_acls);
void nbrec_port_group_index_set_external_ids(const struct nbrec_port_group *,
const struct smap *);
void nbrec_port_group_index_set_name(const struct nbrec_port_group *,
const char *name);
void nbrec_port_group_index_set_ports(const struct nbrec_port_group *,
struct nbrec_logical_switch_port **ports, size_t n_ports);

/* QoS table. */
struct nbrec_qos {
...
#define nbrec_qos_col_bandwidth (nbrec_qos_columns[NBREC_QOS_COL_BANDWIDTH])
...
const struct nbrec_qos_table *nbrec_qos_table_get(const struct ovsdb_idl *);
const struct nbrec_qos *nbrec_qos_table_first(const struct nbrec_qos_table *);

#define NBREC_QOS_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = nbrec_qos_table_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_qos_next(ROW))
#define NBREC_QOS_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = nbrec_qos_table_first(TABLE); \
             (ROW) ? ((NEXT) = nbrec_qos_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

const struct nbrec_qos *nbrec_qos_get_for_uuid(const struct ovsdb_idl *, const struct uuid *);
const struct nbrec_qos *nbrec_qos_table_get_for_uuid(const struct nbrec_qos_table *, const struct uuid *);
...
const struct nbrec_qos *nbrec_qos_table_track_get_first(const struct nbrec_qos_table *);
#define NBREC_QOS_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = nbrec_qos_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_qos_track_get_next(ROW))
...
struct nbrec_qos *nbrec_qos_index_find(struct ovsdb_idl_index *, const struct nbrec_qos *);

int nbrec_qos_index_compare(
    struct ovsdb_idl_index *, 
    const struct nbrec_qos *, 
    const struct nbrec_qos *);
struct ovsdb_idl_cursor nbrec_qos_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor nbrec_qos_cursor_first_eq(
    struct ovsdb_idl_index *, const struct nbrec_qos *);
struct ovsdb_idl_cursor nbrec_qos_cursor_first_ge(
    struct ovsdb_idl_index *, const struct nbrec_qos *);

struct nbrec_qos *nbrec_qos_cursor_data(struct ovsdb_idl_cursor *);

#define NBREC_QOS_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_qos_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = nbrec_qos_cursor_data(&cursor__), \
                  !(TO) || nbrec_qos_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define NBREC_QOS_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_qos_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = nbrec_qos_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define NBREC_QOS_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_qos_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = nbrec_qos_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
void nbrec_qos_verify_bandwidth(const struct nbrec_qos *);
...
const struct ovsdb_datum *nbrec_qos_get_bandwidth(const struct nbrec_qos *, enum ovsdb_atomic_type key_type, enum ovsdb_atomic_type value_type);
...
void nbrec_qos_set_action(const struct nbrec_qos *, const char **key_action, const int64_t *value_action, size_t n_action);
void nbrec_qos_set_bandwidth(const struct nbrec_qos *, const char **key_bandwidth, const int64_t *value_bandwidth, size_t n_bandwidth);
...
void nbrec_qos_add_clause_action(struct ovsdb_idl_condition *, enum ovsdb_function function, const char **key_action, const int64_t *value_action, size_t n_action);
void nbrec_qos_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_qos_update_bandwidth_setkey(const struct nbrec_qos *,  const char *, int64_t );
void nbrec_qos_update_bandwidth_delkey(const struct nbrec_qos *,  const char *);
void nbrec_qos_add_clause_bandwidth(struct ovsdb_idl_condition *, enum ovsdb_function function, const char **key_bandwidth, const int64_t *value_bandwidth, size_t n_bandwidth);
void nbrec_qos_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
...
struct nbrec_qos *nbrec_qos_index_init_row(struct ovsdb_idl_index *);
void nbrec_qos_index_set_action(const struct nbrec_qos *,
const char **key_action, const int64_t *value_action, size_t n_action);
void nbrec_qos_index_set_bandwidth(const struct nbrec_qos *,
const char **key_bandwidth, const int64_t *value_bandwidth, size_t n_bandwidth);
...
const struct nbrec_ssl_table *nbrec_ssl_table_get(const struct ovsdb_idl *);
const struct nbrec_ssl *nbrec_ssl_table_first(const struct nbrec_ssl_table *);

#define NBREC_SSL_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = nbrec_ssl_table_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_ssl_next(ROW))
#define NBREC_SSL_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = nbrec_ssl_table_first(TABLE); \
             (ROW) ? ((NEXT) = nbrec_ssl_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

const struct nbrec_ssl *nbrec_ssl_get_for_uuid(const struct ovsdb_idl *, const struct uuid *);
const struct nbrec_ssl *nbrec_ssl_table_get_for_uuid(const struct nbrec_ssl_table *, const struct uuid *);
...
const struct nbrec_ssl *nbrec_ssl_table_track_get_first(const struct nbrec_ssl_table *);
#define NBREC_SSL_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = nbrec_ssl_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_ssl_track_get_next(ROW))
...
struct nbrec_ssl *nbrec_ssl_index_find(struct ovsdb_idl_index *, const struct nbrec_ssl *);

int nbrec_ssl_index_compare(
    struct ovsdb_idl_index *, 
    const struct nbrec_ssl *, 
    const struct nbrec_ssl *);
struct ovsdb_idl_cursor nbrec_ssl_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor nbrec_ssl_cursor_first_eq(
    struct ovsdb_idl_index *, const struct nbrec_ssl *);
struct ovsdb_idl_cursor nbrec_ssl_cursor_first_ge(
    struct ovsdb_idl_index *, const struct nbrec_ssl *);

struct nbrec_ssl *nbrec_ssl_cursor_data(struct ovsdb_idl_cursor *);

#define NBREC_SSL_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_ssl_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = nbrec_ssl_cursor_data(&cursor__), \
                  !(TO) || nbrec_ssl_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define NBREC_SSL_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_ssl_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = nbrec_ssl_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define NBREC_SSL_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_ssl_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = nbrec_ssl_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
struct nbrec_ssl *nbrec_ssl_index_init_row(struct ovsdb_idl_index *);
